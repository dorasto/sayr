...
description: Personal (cross-org) saved views on the board — the global store, the three different view-switcher UIs and what each owns, dirty-state detection/reset/update, and the Favourites sidebar — use whenever touching personal-views-store, preset-switcher, active-view-switcher, save/edit-view popovers, or favourites-section.tsx
metadata:
  audience: developers
...
A personal saved view is a `saved_view` row with `organizationId: null` — a snapshot of `filterParams` + `viewConfig` (grouping/sort/view-mode/icon/color) scoped to the signed-in user, not an org. They power three separate pieces of UI, each with a **deliberately different job** — the recurring mistake this skill exists to prevent is assuming they're redundant and merging their responsibilities:

| UI | File | Owns |
|---|---|---|
| Top-bar / side-panel switcher | `saved-views/preset-switcher.tsx` (`PresetSwitcher`, `PresetSwitcherContent`) | Full view management: select, pin/unpin, **edit** (rename + icon/color), **delete**. Lives wherever `landerLayout` puts it — see the `board` skill. |
| Breadcrumb switcher | `saved-views/active-view-switcher.tsx` (`ActiveViewSwitcher`) | Select + **dirty-state detection** + Reset/Update only. Deliberately does *not* duplicate pin/edit/delete — one owner for those actions avoids two UIs drifting out of sync. |
| Sidebar shortcuts | `admin/sidebars/favourites-section.tsx` (`FavouritesSection`) | Pinned views only, rendered on **every** admin page (not just `/home`), navigate-to-and-apply. |

All three read the same global `personal-views-store.ts` — there's exactly one source of truth, not per-component fetches, specifically because the Favourites sidebar needs the list on pages that never mount `/home`'s own data-loading context.
...
`ActiveViewSwitcher` computes `isDirty = !!activeView && !areStatesEqual({ filters, viewConfig }, getViewCombinedState(activeView))` and shows a small dot + "Reset changes" (`resetToSavedView`) / "Update view" (`usePersonalViews().updateView` with `serializeFilters(filters)` + `mapStateToViewConfig(viewConfig, activeView's icon/color)`) once dirty.

**Quick-filter chips clear the active view entirely** (`applyFilter` passes `{ view: null, ... }` in its URL params) — they were never designed to layer on top of a saved view, so clicking one while a view is active is not a way to "make it dirty," it's a way to leave the view. Use the View-options popover (grouping/sort/show-completed — these call `setViewConfig` directly, no URL `view` param touched) to test/reproduce dirty-state instead.
...
`admin/sidebars/favourites-section.tsx`, mounted in `primary.tsx` between the nav-items group and Organizations — hidden entirely when nothing's pinned. Drag-reorder uses a dedicated grip handle (not the whole row), since the row itself is a real `<Link>`.
...
1. **Don't add pin/edit/delete to `ActiveViewSwitcher`** — that's `PresetSwitcher`'s job. If you need those actions reachable from the breadcrumb, add a way to open `PresetSwitcher`'s surface, don't duplicate its logic.
