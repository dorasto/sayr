 ...
 * task detail view sidebar. Uses the default (small/cheap) model on the
 * plain chat path. When URL fetching is enabled, external URLs found in task
 * content are fetched server-side and their text folded into the prompt —
 * see `fetchUrlAsText` in apps/backend and `resolveModelId`/`REQUESTY_MODEL_CATALOG`
 * in `@repo/ai` for the Pro-tier model picker that can override `model` below.
 */
