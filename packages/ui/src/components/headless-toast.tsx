...
let headlessToastIcons: Partial<Record<HeadlessToastVariant, React.ReactNode | null>> = {};

export function setHeadlessToastIcons(map: Partial<Record<HeadlessToastVariant, React.ReactNode | null>>) {
...
export function HeadlessToast(props: Required<Pick<HeadlessToastOptions, "id">> & HeadlessToastOptions) {
	const { id, title, description, action, icon, variant = "default", className } = props;
		...
		>
			<div className="flex-1 space-y-1">
				<div className="flex items-center gap-2">
					{resolvedIcon ? <div className="mt-0.5 shrink-0">{resolvedIcon}</div> : null}
					{title ? <p className={cn("text-sm font-bold leading-none")}>{title}</p> : null}
				</div>
...
headlessToast.info = (options: Omit<HeadlessToastOptions, "variant">) => headlessToast({ variant: "info", ...options });
