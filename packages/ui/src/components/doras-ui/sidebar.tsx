...
const SIDEBAR_MIN_DRAG_WIDTH = 144;
...
function SidebarResizeHandle({
  ...
  const resizeTo = (width: number) => {
    const maxWidth = Math.min(
      SIDEBAR_MAX_RESIZED_WIDTH,
      window.innerWidth - SIDEBAR_MIN_CONTENT_WIDTH,
    );
    return Math.round(
      Math.min(maxWidth, Math.max(SIDEBAR_MIN_DRAG_WIDTH, width)),
    );
  };
  ...
  const handlePointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const previousWidth =
      document.documentElement.style.getPropertyValue(widthProperty);
  ...
  const handleKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    const nextWidth = resizeTo(
      aside.getBoundingClientRect().width +
        (side === "left" ? direction : -direction) * step,
    );
    if (nextWidth <= SIDEBAR_COLLAPSE_THRESHOLD) {
      sidebarActions.collapseSidebar(id);
      return;
...
export function Sidebar({
  ...
  React.useEffect(() => {
    document.documentElement.style.setProperty(
      `--sidebar-${id}-width`,
      isDockedOpen ? expandedWidth : "0px",
    );
  }, [expandedWidth, id, isDockedOpen]);

  const baseStyles =
    "sticky top-0 h-full overflow-hidden transition-[width] data-[resizing=true]:transition-none";
        ...
        {isDockedOpen && (
          <div className={cn(variantRootStyles[variant], rootClassName)}>
            ...
            >
              <div
                className="flex h-full flex-col overflow-hidden"
                style={{ width: renderedDockedWidth }}
              >
                {children}
              </div>
...
export function SidebarSubmenu({
  ...
  const trigger = (
    ...
    >
      ...
      <IconChevronRight
        className={cn("transition-all size-4", isOpen && "rotate-90")}
      />
    </button>
  );
    ...
    <Button
      ...
      onPointerEnter={(event) => {
        ...
        if (!isMobile && !isDockedOpen)
          sidebarActions.scheduleSidebarOverlayOpen(sidebarId);
      }}
