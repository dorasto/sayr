...
// The dimming scrim for a `modal` IndentDrawer — absolutely (not fixed)
// positioned to stay confined to whatever local container it's portalled
// into (see the top-of-file comment on staying off `position: fixed`),
// matching IndentDrawerIndentBackground's own `absolute inset-0`. Only
// rendered by IndentDrawerContent when its own `modal` prop is true —
// non-modal drawers (the default) render no backdrop at all, same as
// before. z-[10000]: above the app's own PageHeader (z-9999, sticky,
// see apps/start's page.tsx) — a modal should dim that too, not just the
...
export function IndentDrawerBackdrop({ className, ...props }: React.ComponentProps<typeof DrawerPrimitive.Backdrop>) {
	return (
		<DrawerPrimitive.Backdrop
			data-slot="indent-drawer-backdrop"
			className={cn(
				"absolute inset-0 z-[10000] bg-black/80 transition-opacity duration-300",
...
export function IndentDrawer({
	...
	swipeDirection,
	side = "right",
	// Non-modal drawers default to true: with the background still
	// interactive, an "outside" pointer down is a normal page interaction,
	// not dismiss-intent. A modal drawer flips this default to false — with
...
export function IndentDrawerContent({
	...
	/**
	 * Where the `modal` backdrop portals to — pass the page's own root
	 * container (not this drawer's local push-region container) so the
	 * scrim covers the whole page, including a sticky header rendered
	 * above this drawer's own confined region. Falls back to `container`
	 * when omitted (backdrop stays scoped to the local region, matching
	 * pre-existing IndentDrawerRegion/IndentBackground behavior). Ignored
	 * when `modal` is false.
	 */
	...
	return (
		<>
			{modal && (
				<DrawerPrimitive.Portal container={backdropContainer ?? container}>
			...
			<DrawerPrimitive.Portal container={container}>
				{/* pointer-events-none on the viewport + pointer-events-auto only on the
				    ...
				    whatever's behind it, no backdrop, no focus trap. A modal drawer
				    (IndentDrawerBackdrop, portalled separately above) gets its focus trap
				    and blocked outside pointer events from Base UI's own `modal` handling
				    on the Root; this viewport's own pointer-events setup is unrelated and
