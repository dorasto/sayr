...
export function Preview({
	...
	const defaultTrigger = (
		<a href={normalizedUrl} className="text-primary hover:underline inline-flex items-center gap-1">
	...
	return (
		<HoverCard>
			<HoverCardTrigger
				delay={200}
				className={className}
				render={(children || defaultTrigger) as React.ReactElement}
			/>
			<HoverCardContent
				className={cn("md:w-80 p-0 overflow-hidden border rounded-lg no-underline! select-none!", contentClassName)}
			>
				<a href={normalizedUrl} className="no-underline! pointer-events-auto! block">
					{loading ? (
