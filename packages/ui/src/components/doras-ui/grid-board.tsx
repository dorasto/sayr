...
>({ children, className, ...props }: GridBoardColumnsProps<TColumn>) {
  ...
  return (
    <div
      className={cn("flex gap-3 sticky top-0 z-30 bg-background", className)}
      {...props}
    >
  ...
  return (
    <div
      className={cn(
        ...
        "flex items-center justify-between px-3.5 py-2.5 bg-background min-w-[280px] flex-1 gap-2 rounded-t-xl",
        className,
      )}
      {...props}
    >
...
>({ children, className, ...props }: GridBoardRowsProps<TRow, TColumn>) {
  ...
  return (
    ...
    <div className={cn("flex flex-col", className)} {...props}>
      {rows.map((row, index) =>
        children(row, columns, index === rows.length - 1),
      )}
    </div>
  );
}
  ...
  return (
    <div
      className={cn(
        ...
        "bg-background border-x border-border/40 sticky top-[40px] z-20 overflow-hidden",
    ...
    >
      {children ?? (
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 sticky left-2.5 w-fit">
          {row.icon && (
            <span className={cn("text-sm", row.accentClassName)}>
              {row.icon}
            </span>
          )}
...
function GridBoardDroppableCell({
  ...
  const cellClassName = cn(
    ...
    hasRows ? "bg-transparent" : "rounded-b-xl rounded-t-none bg-background",
  ...
  if (usesCustomScrollbar) {
    return (
      <ScrollArea ref={setNodeRef} className={cellClassName}>
        <div className="flex min-h-full flex-col gap-2.5 px-2.5 py-3 bg-background">
          {children}
        </div>
      </ScrollArea>
    );
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn(cellClassName, "flex flex-col gap-2.5 p-2.5")}
    >
      {children}
    </div>
  );
