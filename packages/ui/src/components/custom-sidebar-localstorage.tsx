...
>(({ defaultOpen, className, style, children, name, ...props }, ref) => {
	const isMobile = useIsMobile();
	const { value: isOpen, setValue: dynamicSetOpen } = useLocalStorage(`${name}-state` || "", defaultOpen);
	React.useEffect(() => {
		dynamicSetOpen(defaultOpen);
	}, [defaultOpen, dynamicSetOpen]);
	...
	const contextValue = React.useMemo<SidebarContext>(
		() => ({
			...
			setOpen: dynamicSetOpen,
		}),
		[isMobile, state, isOpen, dynamicSetOpen, defaultOpen]
	);
	return (
...
>(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
	...
	if (collapsible === "none") {
		return (
			<div
				className={cn(
					"flex h-full w-(--sidebar-width-icon) flex-col bg-sidebar text-sidebar-foreground md:w-(--sidebar-width)",
					className
				)}
				ref={ref}
				{...props}
		...
		return (
			<Drawer open={open} onOpenChange={setOpen}>
				<DrawerContent
					...
					data-mobile="true"
					className="z-[999] mx-auto h-[60%] w-full bg-sidebar p-0 px-4 text-sidebar-foreground [&>button]:hidden"
				>
	...
	return (
		<div
			ref={ref}
			className="group peer hidden w-fit text-sidebar-foreground md:block"
			data-state={state}
		...
		>
			...
			<div
				className={cn(
					"relative inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-200  md:flex",
					side === "left"
						? "left-0 group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)]"
						: "right-0 group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)]",
					// Adjust the padding for floating and inset variants.
					variant === "floating" || variant === "inset"
						? "max-h-[calc(100dvh_-_2.5rem)] p-2 group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)_+_theme(spacing.4)_+2px)]"
	...
	({ className, ...props }, ref) => {
		return (
			<Input
				...
				data-sidebar="input"
				className={cn(
					"h-8 w-full bg-background shadow-none focus-visible:ring-0 focus-visible:ring-transparent",
					className
				)}
				{...props}
			/>
		...
		return (
			<Comp
				...
				className={cn(
					...
					"group-data-[collapsible=icon]:-mt-8 group-data-[state=collapsed]:hidden group-data-[collapsible=icon]:opacity-0",
...
const sidebarMenuButtonVariants = cva(
	"peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:!p-2 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 transition-all",
	{
		variants: {
			variant: {
				default: "hover:bg-sidebar-accent text-sidebar-foreground/70 hover:text-sidebar-accent-foreground",
				outline:
	...
	return (
		<Comp
			...
			className={cn(
				...
				"data-[active=true]:bg-sidebar-accent data-[active=true]:text-sidebar-accent-foreground text-sidebar-foreground/70 select-none",
				size === "sm" && "text-xs",
				size === "md" && "text-sm",
				"group-data-[collapsible=icon]:hidden",
				className
