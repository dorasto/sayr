...
function ComboBoxTrigger({ children, className, disabled, asChild = false }: ComboBoxTriggerProps) {
	...
	if (!isMobile) {
		if (asChild) {
			return <PopoverTrigger render={children as React.ReactElement} />;
		}
		return (
			<PopoverTrigger
				render={
...
function ComboBoxSearch({ placeholder = "Search...", className, icon, onValueChange }: ComboBoxSearchProps) {
	return <CommandInput icon={icon} placeholder={placeholder} className={className} onValueChange={onValueChange} />;
}
...
function ComboBoxEmpty({ children, className }: ComboBoxEmptyProps) {
	return <CommandEmpty className={cn("p-3", className)}>{children}</CommandEmpty>;
}
