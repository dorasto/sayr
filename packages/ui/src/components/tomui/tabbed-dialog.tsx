...
export function TabbedDialog({
	...
	const renderSideTabItem = (tab: Tab) => {
		...
		if (tab.href) {
			return (
				<a key={tab.id} href={tab.href} className={cn(baseClasses, variantClasses, "text-muted-foreground")}>
					{content}
				</a>
			);
		}
						...
						{groups
							? groups.map((group) => (
									<div key={group.id} className="">
										...
										<div className="space-y-1">{group.tabs.map((tab) => renderSideTabItem(tab))}</div>
									</div>
								))
	...
	const renderMobileDropdownItem = (tab: Tab) => {
		...
		if (tab.href) {
			return (
				<DropdownMenuItem
					key={tab.id}
					render={<a href={tab.href}>{content}</a>}
