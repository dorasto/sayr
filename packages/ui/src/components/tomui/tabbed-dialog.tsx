...
export function TabbedDialog({
  ...
  const [tabFooters, setTabFooters] = useState<Map<string, ReactNode>>(
    new Map(),
  );
  ...
  const allTabs = useMemo(() => {
    const flatTabs =
      groupedTabsProp && groupedTabsProp.length > 0
        ? groupedTabsProp.flatMap((group) => group.items)
        : tabs;
  ...
  const sizeClasses = {
    ...
    lg:
      layout === "side" ? "max-w-none md:max-w-4xl" : "max-w-none md:max-w-4xl",
    xl:
      layout === "side" ? "max-w-none md:max-w-5xl" : "max-w-none md:max-w-5xl",
  };

  const renderTopTabs = () => (
    <DialogHeader className="flex-shrink-0 border-b px-0 pt-1 space-y-3 relative bg-background transition-all">
      ...
      <div className="relative">
        ...
        <div
          ...
          className="w-full overflow-x-auto scrollbar-hide px-3"
        >
  ...
  const renderSideTabs = () => {
    ...
    return (
      <div className="flex-shrink-0 w-52 bg-popover overflow-hidden transition-all">
        <div className="p-4">
            ...
            {layout === "side" && stickyHeader && (
              <div className="p-4 w-full">
                {(() => {
                  ...
                  const activeTabData = allTabs.find(
...
export function TabPanel({
  ...
  return (
    <div
      className={cn("p-4", shouldAddScrolling && "overflow-y-auto", className)}
    >
      {children}
    </div>
  );
