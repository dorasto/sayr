...
app.get("/", (c) => c.text("ok", { status: 200 }));
...
app.get("/ui/:id/pages", async (c) => {
  ...
  return c.json({
    ...
    data: {
      ...
      authorName: integration.author?.name ?? null,
      authorUrl: integration.author?.url ?? null,
...
(async () => {
  ...
  for (const integration of list) {
    ...
    if (integration.api) {
      ...
      console.log(
        `${tag} Route registered -> /:orgId/integrations/${integration.id}`,
      );
    }
  }
