...
export function meta({ params }: Route.MetaArgs) {
  ...
  return [
    ...
    { name: "description", content: issue ? issue.description : "Bug not found" },
  ];
}
    ...
    <div className="p-4 grid gap-4">
      ...
      <div className="grid gap-4">
        ...
        {issue.comments.map((comment) => (
          ...
          <Card className={cn(!comment.public && "bg-secondary text-secondary-foreground")}>
            <CardHeader>
                <div className="flex items-center gap-4">
                    <p className="font-semibold">{comment.author}
                    {comment.usertype === 'internal' && <Badge variant="outline" className="ml-2">Admin</Badge>}
                    {comment.public === false && <Badge variant="outline" className="ml-2">Private</Badge>}
                    </p>
