...
import { getReleaseStatusConfig, RELEASE_STATUS_ORDER, type ReleaseStatusKey } from "@/components/releases/config";
...
export const Route = createFileRoute("/orgs/$orgSlug/releases/")({
	loader: async ({ params, context }) =>
		fetchPublicReleases({
			data: { slug: (context as { systemSlug?: string | null })?.systemSlug || params.orgSlug },
		}),
...
function ReleaseDateLabel({ release }: { release: schema.releaseType }) {
	if (release.status === "released" && release.releasedAt) {
		return <span className="text-xs text-muted-foreground">Released {formatDate(release.releasedAt)}</span>;
	}
	if (release.targetDate) {
		return <span className="text-xs text-muted-foreground">Target {formatDate(release.targetDate)}</span>;
	}
	const cfg = getReleaseStatusConfig(release.status);
	return <span className="text-xs text-muted-foreground">{cfg?.label ?? release.status}</span>;
}
...
function ReleasesListPage() {
	...
	const grouped = RELEASE_STATUS_ORDER.filter((s) => s !== "archived").map((status) => ({
		status,
	...
	return (
		<div className="max-w-3xl mx-auto px-4 py-6 space-y-8">
			...
			{grouped.map(({ status, releases: groupReleases }) => {
				const cfg = getReleaseStatusConfig(status as schema.releaseType["status"]);
				return (
					<section key={status}>
						<div className="flex items-center gap-2 mb-3">
							<span className={cn("flex items-center gap-1.5 text-sm font-semibold", cfg?.className)}>
								{cfg?.icon("h-4 w-4")}
								{cfg?.label}
							</span>
							<span className="text-xs text-muted-foreground">({groupReleases.length})</span>
						</div>
			...
			{archived.length > 0 && (
				<section>
					...
					>
						...
						<IconChevronDown
							className={cn("h-3 w-3 transition-transform", archivedOpen && "rotate-180")}
						/>
					</button>
					{archivedOpen && (
...
function ReleaseCard({ release, orgSlug }: { release: schema.releaseType; orgSlug: string }) {
	const cfg = getReleaseStatusConfig(release.status);
	...
	return (
		...
		>
			...
			<div className="flex flex-1 flex-col gap-0.5 min-w-0">
				<span className="font-semibold text-sm group-hover:text-foreground">{release.name}</span>
			</div>
