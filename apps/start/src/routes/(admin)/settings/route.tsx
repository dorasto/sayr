...
function SettingsHeader() {
  ...
  const integrationMatch = useMatch({
    from: "/(admin)/settings/org/$orgId/connections/$integrationId/",
    shouldThrow: false,
  });

  const organization = orgSettingsMatch?.loaderData?.organization;
  const orgId = orgSettingsMatch?.params?.orgId;
  const integrationName = integrationMatch?.loaderData?.name;
      ...
      <PageHeader>
        <PageHeader.Identity>
          ...
          {integrationName && (
            <>
              <span className="text-muted-foreground text-xs">/</span>
              <span className="text-xs px-1">{integrationName}</span>
            </>
          )}
        </PageHeader.Identity>
      </PageHeader>
