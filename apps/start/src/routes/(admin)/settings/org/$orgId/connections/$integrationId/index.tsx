	...
	.handler(async ({ data }) => {
		const API_URL =
			process.env.APP_ENV === "development"
				...
				: "http://localhost:5468/api/internal";
		try {
			const res = await fetch(`${API_URL}/v1/admin/integrations/ui/${data.integrationId}/pages`);
...
export const Route = createFileRoute("/(admin)/settings/org/$orgId/connections/$integrationId/")({
	loader: async ({ params }) => {
...
function RouteComponent() {
	...
	const { data: pagesData, isLoading } = useQuery({
		...
		queryFn: async () => {
			const res = await fetch(`${base}/v1/admin/integrations/ui/${integrationId}/pages`, { credentials: "include" });
	...
	const { data: listData } = useQuery({
		...
		queryFn: async () => {
			const res = await fetch(`${base}/v1/admin/integrations/list?orgId=${orgId}`, { credentials: "include" });
			if (!res.ok) throw new Error("Failed to fetch");
			return res.json();
		},
	});
	...
	const integrationEnabled: boolean =
		(listData?.data as Array<{ id: string; enabled: boolean }> | undefined)?.find((i) => i.id === integrationId)
			?.enabled ?? false;

	const enableMutation = useMutation({
		mutationFn: async (enable: boolean) => {
			const action = enable ? "enable" : "disable";
			const res = await fetch(`${base}/v1/admin/integrations/${orgId}/${integrationId}/${action}`, {
				method: "POST",
				credentials: "include",
			});
				...
				<div className="flex items-center gap-2">
					<Toggle
						...
						className={cn(
							...
							integrationEnabled ? "bg-accent" : "bg-muted text-muted-foreground"
						)}
					>
						<IconCircleFilled size={8} className={integrationEnabled ? "text-primary" : "text-destructive"} />
