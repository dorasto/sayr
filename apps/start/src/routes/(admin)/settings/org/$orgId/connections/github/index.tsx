...
import { IconBrandGithub, IconPlus, IconQuestionMark } from "@tabler/icons-react";
...
export const Route = createFileRoute("/(admin)/settings/org/$orgId/connections/github/")({
...
function RouteComponent() {
	...
	const { data: githubData } = useQuery({
		...
		queryFn: async () => {
			const res = await fetch(`${base}/v1/admin/organization/${orgId}/connections/github`, {
				credentials: "include",
			});
	...
	const githubConnections: githubConnections = githubData?.data?.githubConnections ?? [];

	const githubConnectionsRepositories: githubConnectionsRepositories = githubData?.data?.repositories ?? [];
	...
	const {
		...
		queryFn: async () => {
			const res = await fetch(`${base}/v1/admin/organization/${orgId}/github/installations`, {
				credentials: "include",
			});
	...
	const linkMutation = useMutation({
		mutationFn: async (installationId: number) => {
			const res = await fetch(`${base}/v1/admin/organization/${orgId}/github/link`, {
				method: "POST",
				credentials: "include",
	...
	const githubAccountNotConnected =
		installationsData?.error && (!installationsData?.data || installationsData.data.length === 0);
					...
					<AdaptiveDialogBody>
						...
						{githubAccountNotConnected ? (
							<div className="flex flex-col gap-4">
								<p className="text-sm text-muted-foreground">Please connect your GitHub account first.</p>
											...
											return (
												...
												>
													<div className="flex items-center gap-3">
														...
														/>
														<div>
															<p className="text-sm font-medium">{install.account.login}</p>
															<p className="text-xs text-muted-foreground">{install.account.type}</p>
														</div>
													</div>
