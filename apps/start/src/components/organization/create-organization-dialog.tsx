							...
							{slugError ? (
								...
								<p className="text-xs text-muted-foreground">
									...
									<code className="bg-muted px-1 py-0.5 rounded">
										{slug || "your-slug"}.{import.meta.env.VITE_ROOT_DOMAIN || "sayr.io"}
									</code>
								</p>
							)}
						</div>
						<div className="flex flex-col gap-2">
							...
							<Input
								...
								onChange={(e) => {
									...
									setShortId(raw.replace(/[^A-Z]/g, "").slice(0, 3));
								}}
								className="bg-accent border-transparent font-mono uppercase"
								maxLength={3}
							/>
							<p className="text-xs text-muted-foreground">
								Used as prefix, e.g.{" "}
								<code className="bg-muted px-1 py-0.5 rounded">{shortId || "SAY"}-123</code>
							</p>
						</div>
					</div>
