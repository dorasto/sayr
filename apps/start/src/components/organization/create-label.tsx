...
import { createLabelAction, deleteLabelAction, editLabelAction } from "@/lib/fetches/organization";
...
export default function CreateLabel({
	...
	const [visible, setVisible] = useState<"public" | "private">(label?.visible ?? "public");
	const { runWithToast, isFetching } = useToastAction();
	const isEditMode = mode === "edit" && label;
	const change = name !== label?.name || color.hsla !== label?.color || visible !== (label?.visible ?? "public");
							...
							render={
								<InputGroupButton
									...
									className={cn(
										...
										visible === "public" ? "border-transparent" : "bg-primary/10 border-primary/50"
									)}
									variant={"ghost"}
									onClick={() => setVisible(visible === "public" ? "private" : "public")}
								>
									{visible === "public" ? <IconEye className="" /> : <IconEyeOff className="" />}
								</InputGroupButton>
							}
						/>
				...
				{!isEditMode ? (
					...
					<InputGroupAddon align="inline-end" className={cn("")}>
						{change ? (
							<InputGroupButton
								variant="ghost"
								className={cn("h-full bg-success/30 hover:bg-success/50 rounded-lg")}
								onClick={async () => {
									const data = await runWithToast(
										"edit-label",
										{
									...
									if (data?.success && data.data) {
										setLabels(data.data);
										const updatedLabel = data.data.find((e) => e.id === label?.id);
