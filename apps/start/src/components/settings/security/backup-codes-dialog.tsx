...
export function BackupCodesDialog({ open, onOpenChange, codes, isSetup = false, onDone }: BackupCodesDialogProps) {
	const handleClose = () => {
		if (onDone) onDone();
		onOpenChange(false);
	};
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-w-md">
				<DialogHeader>
					<DialogTitle>{isSetup ? "Save Your Backup Codes" : "Backup Codes"}</DialogTitle>
					<DialogDescription>
						{isSetup
				...
				<div className="py-4 flex flex-col gap-4">
					{isSetup && (
						<div className="flex items-start gap-2 rounded-md border border-yellow-500/30 bg-yellow-500/10 px-3 py-2.5">
							...
							<p className="text-sm text-yellow-800 dark:text-yellow-200">
								If you lose access to your authenticator app and don't have these codes, you may be locked out
								of your account.
							</p>
						</div>
					)}
					<div className="grid grid-cols-2 gap-2 font-mono text-sm">
						{codes.map((code) => (
							<div key={code} className="p-2 bg-accent rounded text-center select-all">
								{code}
							</div>
						))}
					</div>
				...
				<DialogFooter>
					...
					<Button variant="outline" onClick={() => handleDownloadBackupCodes(codes)}>
						<IconDownload className="size-4 mr-1" />
						Download
					</Button>
					<Button onClick={handleClose}>{isSetup ? "I've saved my codes" : "Close"}</Button>
				</DialogFooter>
			</DialogContent>
		</Dialog>
	);
