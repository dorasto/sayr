...
export function TwoFactorSetupDialog({ open, onOpenChange, totpUri, onVerified }: TwoFactorSetupDialogProps) {
	...
	useEffect(() => {
		if (totpUri) {
			QRCode.toDataURL(totpUri, { width: 200, margin: 2 }).then(setQrCodeUrl).catch(console.error);
		}
	}, [totpUri]);
				...
				<div className="py-4 flex flex-col items-center gap-4">
					{qrCodeUrl && (
						<div className="flex flex-col items-center gap-2">
							...
							<p className="font-mono text-sm bg-accent px-3 py-1.5 rounded select-all">{getTotpSecret()}</p>
						</div>
					)}
