...
import { IconCalendarCheck, IconCalendarEvent } from "@tabler/icons-react";
...
export interface ReleaseHoverCardProps
	extends Pick<
		HoverCardBaseProps,
...
function TaskDonutChart({ pct }: { pct: number }) {
	...
	return (
		<ChartContainer config={donutChartConfig} className="w-5 h-5 shrink-0 aspect-square">
			<PieChart>
				<Pie
					data={data}
					dataKey="value"
...
export function ReleaseHoverCard({
	...
	const descText = release.description ? extractTextFromNode(release.description).trim() : "";
					...
					<Tile className="p-0 md:w-full bg-transparent flex-col gap-2 items-start">
						<TileHeader className="w-full">
							<div className="flex items-center gap-1 w-full">
								<TileIcon>
									{release.icon ? (
										<RenderIcon iconName={release.icon} size={14} color={release.color || undefined} raw />
									) : (
										<div
				...
				<div className="flex flex-wrap gap-2 w-full">
					...
					{statusConfig && (
						<InlineLabel
							...
							className={cn("ps-7 h-6 bg-accent rounded-2xl pe-3 border", statusConfig.badgeClassName)}
						/>
					)}
					{/* Date */}
					{displayDate && (
						<InlineLabel
							...
							icon={
								isReleased ? <IconCalendarCheck className="size-5" /> : <IconCalendarEvent className="size-5" />
							}
