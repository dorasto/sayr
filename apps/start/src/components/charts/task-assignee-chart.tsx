...
function DonutChart({ percentage }: { percentage: number }) {
	...
	return (
		<ChartContainer config={chartConfig} className="w-8 h-8 shrink-0 aspect-square">
			<PieChart>
				<Pie
					data={chartData}
					dataKey="value"
...
export function TaskAssigneeChart({ tasks, maxItems = 8, className, renderTileWrapper }: TaskAssigneeChartProps) {
		...
		<div className={cn("w-full", className)}>
			<div className="space-y-1 w-full">
				{assigneeData.map((assignee) => {
					const percentage = assignee.totalCount > 0 ? (assignee.completedCount / assignee.totalCount) * 100 : 0;
					...
					const tileContent = (
						...
						>
							<TileHeader>
								...
								<TileTitle className="flex items-center gap-2">{assignee.name}</TileTitle>
							</TileHeader>
