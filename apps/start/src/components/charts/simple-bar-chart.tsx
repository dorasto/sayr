		...
		return (
			<ChartContainer config={chartConfig} className={cn("w-full", sizeConfig.container, className)}>
				...
				>
					...
					<YAxis dataKey={nameKey} type="category" tickLine={false} tickMargin={8} axisLine={false} hide />
								...
								formatter={(value, _name, item) => (
									<>
										...
										<div className="flex flex-1 items-center justify-between gap-2">
											<span className="text-muted-foreground">{item.payload[nameKey]}</span>
		...
		<ChartContainer config={chartConfig} className={cn("w-full", sizeConfig.container, className)}>
			<BarChart accessibilityLayer data={chartData} margin={{ top: 20 }}>
				...
				<Bar dataKey={dataKey} radius={radius}>
					{showLabels && <LabelList dataKey={dataKey} position="top" fill="var(--foreground)" fontSize={12} />}
				</Bar>
			</BarChart>
		</ChartContainer>
	);
