...
import { extractHslValues, formatTaskKey } from "@repo/util";
...
export function PublicSearchDialog({ open, onOpenChange }: PublicSearchDialogProps) {
	...
	const filteredReleases = React.useMemo(() => {
		...
		return releases.filter((r) => r.name.toLowerCase().includes(lower)).slice(0, 5);
	}, [search, releases]);

	const hasTaskResults = taskResults.length > 0;
	const hasReleaseResults = filteredReleases.length > 0;
	const isEmpty = !isSearchingTasks && !hasTaskResults && !hasReleaseResults && search.length >= 2;
	const isIdle = search.length < 2;

	return (
		<CommandDialog open={open} onOpenChange={onOpenChange} showOverlay={false}>
			<DialogTitle className="sr-only">Search</DialogTitle>
			<CommandInput placeholder="Search tasks & releases..." value={search} onValueChange={setSearch} />
			<CommandList>
				{isEmpty && <CommandEmpty>No results found.</CommandEmpty>}
				{isIdle && !hasReleaseResults && <CommandEmpty>Type at least 2 characters to search.</CommandEmpty>}
										...
										<span
											...
											style={{
												background: release.color
													? `${release.color}20`
													: `hsla(${extractHslValues(cfg?.hsla ?? "0,0%,50%,1")}, 0.1)`,
											}}
										>
											<RenderIcon
