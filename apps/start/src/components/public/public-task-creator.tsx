...
export function PublicTaskCreator() {
	...
	const { organization, labels, categories, issueTemplates } = usePublicOrganizationLayout();
	...
	const [templateData, setTemplateData] = useState<NodeJSON | undefined>(undefined);
	...
	const handleTemplateSelect = useCallback(
		(templateId: string) => {
			...
			if (template) {
				...
				if (template.priority && fields.priority) setPriority(template.priority);
				if (template.categoryId && fields.category) setSelectedCategory(template.categoryId);
	...
	const handleSubmit = useCallback(async () => {
		...
		try {
			...
			if (finalDescription) {
				finalDescription = await processUploads(finalDescription, "public", organization.id, "public-task-create");
			}
			...
			<CollapsibleContent className="overflow-hidden data-closed:animate-collapsible-up data-open:animate-collapsible-down">
				<div className="bg-accent overflow-hidden rounded-lg">
					...
					<div className="flex items-center gap-2 p-3 pb-0">
									...
									<ComboBoxValue placeholder="Choose a template...">
										{selectedTemplateId === "__none__"
											? templateRequired
												...
												: "Template"
											: issueTemplates.find((t) => t.id === selectedTemplateId)?.name || "Template"}
									</ComboBoxValue>
								</ComboBoxTrigger>
								<ComboBoxContent>
									...
									<ComboBoxList>
										...
										<ComboBoxGroup>
											{!templateRequired && <ComboBoxItem value="__none__">No template</ComboBoxItem>}
						...
						<Button variant="ghost" size="icon" className="h-7 w-7 ml-auto" onClick={() => setOpen(false)}>
					...
					{(fields.priority || fields.labels || fields.category) && (
						<div className="flex items-center gap-2 px-3 py-2 flex-wrap">
							{/* Priority picker */}
							{fields.priority && (
								<ComboBox value={priority} onValueChange={(val) => setPriority(val || "none")}>
							...
							{fields.category && availableCategories.length > 0 && (
								<ComboBox value={selectedCategory} onValueChange={(val) => setSelectedCategory(val || "")}>
									<ComboBoxTrigger className="w-fit text-xs h-7 border border-transparent hover:border-border bg-secondary text-accent-foreground hover:bg-secondary rounded-lg px-2 flex items-center gap-2">
										<ComboBoxValue placeholder="Category">
											{selectedCategory
												? availableCategories.find((c) => c.id === selectedCategory)?.name || "Category"
												: "Category"}
										</ComboBoxValue>
									</ComboBoxTrigger>
									<ComboBoxContent>
					...
					<div className="flex items-center justify-between gap-2 p-3 pt-0">
						<p className="text-xs text-muted-foreground">Your task will be publicly visible.</p>
						<Button
							variant="primary"
							className="bg-secondary"
							size="sm"
							onClick={handleSubmit}
							disabled={isSubmitting || !title.trim() || (templateRequired && selectedTemplateId === "__none__")}
						>
							{isSubmitting ? (
								<>
