...
// Not switched to @repo/util's formatDate/formatDateCompact: those use
// day: "numeric" (no leading zero) and don't uppercase the result, whereas
// this reads "SEP 18, 2025" (day: "2-digit" + .toUpperCase()) for the
// timeline date column — switching would silently change the displayed text.
...
export function ReleaseCard({ release, orgSlug, lineColor, isLast }: ReleaseCardProps) {
	...
	const primaryDate =
		release.status === "released" && release.releasedAt ? release.releasedAt : (release.targetDate ?? null);

	const dateLabel = release.status === "released" ? "Released" : release.targetDate ? "Target" : null;
	...
	return (
		<div className="flex gap-4 md:gap-6">
			...
			<div className="hidden md:flex flex-col items-end justify-start w-32 shrink-0 gap-0.5 text-right my-auto">
				{primaryDate ? (
					<>
						{dateLabel && (
						...
						<span className="text-[11px] font-medium text-muted-foreground">{formatDate(primaryDate)}</span>
					</>
				) : (
			...
			<div className="hidden md:flex shrink-0 flex-col items-center">
				...
				<div className="w-px grow" style={{ backgroundColor: color, opacity: 0.2 }} />
				{/* Dot */}
				<div className="size-2.5 rounded-full shrink-0 z-10 my-1" style={{ backgroundColor: color }} />
				{/* Line segment below dot — hidden for last card */}
				<div className={cn("w-px grow")} style={{ backgroundColor: color, opacity: 0.2 }} />
			</div>

			{/* Card */}
			<div className="flex-1 min-w-0 py-3">
				<Tile asChild className="md:w-full hover:bg-secondary cursor-pointer p-5 rounded-xl">
					<Link to="/orgs/$orgSlug/releases/$releaseSlug" params={{ orgSlug, releaseSlug: release.slug }}>
						<TileHeader className="items-start gap-2">
							<div className="flex items-center gap-2.5 w-full">
								<TileIcon
									...
									style={{
										...
										color:
											release.color && release.color !== "hsla(0, 0%, 0%, 1)" ? release.color : cfg?.color,
									}}
								>
									{release.icon ? (
										<RenderIcon
							...
							<div className="flex md:hidden items-center gap-2 flex-wrap">
								<Badge variant="outline" className={cn("text-[10px] px-1.5 py-0", cfg?.badgeClassName)}>
									{cfg?.label}
								</Badge>
								{primaryDate && (
									<span className="text-[11px] text-muted-foreground">{formatDate(primaryDate)}</span>
								)}
							</div>

							{descriptionPreview && (
								<TileDescription className="line-clamp-2 text-sm">{descriptionPreview}</TileDescription>
							)}
						</TileHeader>
					</Link>
				</Tile>
