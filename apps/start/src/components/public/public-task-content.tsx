...
export function PublicTaskContent({ task: initialTask, release }: PublicTaskContentProps) {
				...
				>
					<TileHeader className="w-full">
						<div className="flex flex-row gap-3 w-full">
							...
							<span className="ml-auto text-sm text-muted-foreground font-medium">{localVoteCount}</span>
						</div>
					</TileHeader>
				</Tile>
	...
	// Panel body depends on live task/vote/release state, unlike the static
	// header above — real deps, not a set-once-on-mount effect. Still gated
	// on isRegistered: Page defers registering the panel to its client-only
	// pass, so this would otherwise race it on first mount.
	useEffect(() => {
		if (!panel.isRegistered) return;
		setPanelContent(PUBLIC_TASK_PANEL_ID, panelBody);
	}, [
		panel.isRegistered,
		setPanelContent,
		task,
