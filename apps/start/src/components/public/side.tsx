...
export default function PublicTaskSide() {
	...
	const isAllTasksActive = filters.groups.length === 0 && !selectedViewSlug && !categorySlug;
	const { data: session, isPending } = authClient.useSession();
	return (
		<div className="flex flex-col gap-3 w-full sticky top-0 pt-3 self-start" ref={stickyRef}>
			{isPending
				? null
				: !session && (
						<LoginDialog
			...
			<div className="flex flex-col gap-0 bg-card rounded-xl">
				...
				<div className="flex flex-col gap-1 p-1">
					...
					>
						<TileHeader className="w-full">
							<div className="flex flex-row gap-3 w-full">
								...
								<TileDescription className="ml-auto">{opentaskCount}</TileDescription>
							</div>
						</TileHeader>
					</Tile>
					...
					{categories.map((category) => {
						...
						const isActive =
							(slug && categorySlug === slug) || serializeFilters(filters) === serializeFilters(categoryFilter);
							...
							>
								<TileHeader className="w-full">
									<div className="flex flex-row gap-3 w-full">
										...
										<TileDescription className="ml-auto">{categoryTaskCount}</TileDescription>
									</div>
								</TileHeader>
							</Tile>
						);
						...
						<TileHeader className="w-full">
							<div className="flex flex-row gap-3 w-full">
								<TileTitle className="flex items-center gap-2 w-full">
									...
									<span className="line-clamp-1 text-xs">Powered by Sayr.io</span>
								</TileTitle>
							</div>
						</TileHeader>
					</Tile>
