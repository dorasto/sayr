...
export function PublicTaskView({ sortBy, setSortBy }: { sortBy: SortOption; setSortBy: (value: SortOption) => void }) {
	const { tasks, categories, setTasks, organization } = usePublicOrganizationLayout();
	...
	const handleVote = async (taskId: string) => {
		...
		queryClient.setQueryData(votesKey, (old: { taskId: string; voteCount: number; count: number }[] | undefined) => {
			if (!old) return old;
			return isVoted ? old.filter((v) => v.taskId !== taskId) : [...old, { taskId, voteCount: 0, count: 1 }];
		});
					...
					<DropdownMenu>
						<DropdownMenuTrigger asChild>
							<Button variant="primary" size="sm">
								<IconFilter2 />
								{!isMobile && <span className="truncate">{getSortLabel(sortBy)}</span>}
							</Button>
						</DropdownMenuTrigger>
						<DropdownMenuContent className="w-56">
							...
							>
								<DropdownMenuRadioItem value="mostPopular">Most popular</DropdownMenuRadioItem>
