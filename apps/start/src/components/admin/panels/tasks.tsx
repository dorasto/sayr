...
import { IconPencil, IconRocket, IconSettings, IconStack2, IconUser, IconUsers, IconX } from "@tabler/icons-react";
...
function TasksPanelContent() {
	...
	const { views, categories, releases } = useLayoutOrganization();
	...
	// Task counts
	const openTaskCount = tasks.filter((t) => t.status !== "done" && t.status !== "canceled").length;
	const openUserTaskCount = tasks.filter(
		(t) => t.status !== "done" && t.status !== "canceled" && t.assignees.some((a) => a.id === account?.id)
	).length;
		...
		<div className="flex flex-col gap-3 w-full relative">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-2 w-full">
				...
				{categories.map((category) => {
					...
					const categoryTaskCount = tasks.filter((t) => t.category === category.id).length;
			...
			<Tabs defaultValue="views" className="w-full p-1 relative bg-card rounded-lg">
				...
				<TabsContent value="views" className="mt-0">
					<div className="flex flex-col gap-0.5">
						{viewsOverLimit && (
							<PlanLimitBanner title="Views limit exceeded" description={getLimitMessage("savedViews")} />
						)}
						{views.map((view) => {
				...
				<TabsContent value="release" className="mt-0">
					<div className="flex flex-col gap-0.5">
						{releases.length === 0 ? (
							<div className="p-3 text-muted-foreground text-sm text-center">No releases yet</div>
						) : (
							releases.map((release) => {
								const releaseFilter = createReleaseFilter(release.id);
								const isActive = currentFiltersSerialized === serializeFilters(releaseFilter);
								...
								return (
									...
									>
										...
										>
											<TileTitle className="flex items-center gap-2 min-w-0">
												<TileIcon
													className={cn("bg-transparent shrink-0")}
													style={{
														background: isActive
										...
										<TileAction className={cn("p-3 transition-all")}>
				...
				<TabsContent value="priority" className="mt-0">
					<div className="flex flex-col gap-0.5">
						{priorityViews.map(({ key, label }) => {
							...
							const isActive = currentFiltersSerialized === serializeFilters(priorityFilter);
