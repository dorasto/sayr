...
import { RELEASE_STATUS_ORDER, releaseStatusConfig } from "@/components/releases/config";
...
function ReleasesListPanelContent() {
	...
	return (
		<div className="flex flex-col gap-3 w-full relative">
						...
						>
							<TileHeader>
								<TileTitle className="flex items-center gap-2">
									<TileIcon
										className={cn(
											isActive ? "text-foreground bg-muted-foreground/20" : "text-muted-foreground",
											config.className
										)}
									>
			...
			<Tabs defaultValue="status" className="w-full p-1 relative bg-card rounded-lg">
										...
										>
											<TileHeader className="h-fit p-0 flex-1 min-w-0">
												<TileTitle className="flex items-center gap-2 min-w-0">
													<span className="truncate min-w-0 text-xs">{release.name}</span>
												</TileTitle>
											</TileHeader>
											<Badge
												className={cn("border rounded-lg text-xs gap-1.5 shrink-0", config.badgeClassName)}
											>
												{config.icon("w-3 h-3")}
												{config.label}
											</Badge>
