...
export function mergeOrAppendCondition(
	...
	if (
		cfg?.multi &&
		["any", "all", "none", "exact"].includes(condition.operator)
	) {
		let merged = false;
			...
			return {
				...
				conditions: g.conditions.map((c) => {
					if (
						...
						merged = true;
						const existing = Array.isArray(c.value)
							? c.value
							: c.value
								? [c.value as string]
								: [];
	...
	return {
		...
		groups:
			filterState.groups.length > 0
				? filterState.groups.map((g, idx) =>
						idx === 0 ? { ...g, conditions: [...g.conditions, condition] } : g,
					)
				: [newGroup],
	};
}
		...
		groups: filterState.groups
			.map((group) => ({
				...
				conditions: group.conditions.map((c) => {
					...
					const current = Array.isArray(c.value)
						? c.value
						: c.value
							? [c.value as string]
							: [];
	...
	return {
		...
		groups: filterState.groups.map((g) => ({
			...
			conditions: g.conditions.map((c) => {
				...
				const newIsMulti = ["any", "all", "none", "exact"].includes(
					newOperator,
				);
