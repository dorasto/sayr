...
export function SettingsSidebar() {
	...
	const [selectedOrgId, setSelectedOrgId] = useState<string | null>(orgIdFromPath || organizations[0]?.id || null);
		...
		<Sidebar id={sidebarId} collapsible keyboardShortcut="b" className="">
				...
				<SidebarMenu>
					<SidebarMenuItem className="min-h-auto">
						<Link className="w-full" to={lastDashboardRoute}>
							<SidebarMenuButton tooltip="Back to dashboard" size="small" icon={<IconArrowLeft size={16} />}>
								<span>Dashboard</span>
							</SidebarMenuButton>
						</Link>
					</SidebarMenuItem>
			...
			<SidebarContent className="flex flex-col gap-3">
							...
							return (
								<SidebarMenuItem key={item.title} isActive={isActive} className="min-h-auto">
									<Link to={item.url} className="w-full">
										<SidebarMenuButton
											size="small"
											icon={<Icon size={16} className={cn(isActive && item.activeClass)} />}
											tooltip={item.title}
										>
				...
				<SidebarGroup className="">
					...
					<SidebarMenu className="gap-0.5">
						{/* Organization Picker */}
						<ComboBox value={selectedOrgId || undefined} onValueChange={handleOrgChange}>
							<SidebarMenuItem className="min-h-auto">
								<ComboBoxTrigger asChild>
									<SidebarMenuButton
										...
										icon={
											selectedOrg ? (
												<Avatar className={cn(isSidebarOpen ? "h-3 w-3" : "h-4! w-4!")}>
													<AvatarImage
														src={selectedOrg.logo ? ensureCdnUrl(selectedOrg.logo) : ""}
														alt={selectedOrg.name}
														className={cn(isSidebarOpen ? "h-3 w-3" : "h-4! w-4!")}
													/>
												...
												<IconUsers className={cn(isSidebarOpen ? "h-3 w-3" : "h-4! w-4!")} />
											)
										}
									...
									if ("label" in entry) {
										...
										return (
											<SidebarGroupLabel
												...
												className={cn("mt-2 first:mt-0", isSidebarOpen ? "" : "hidden")}
											>
												{labelEntry.label}
											</SidebarGroupLabel>
										);
