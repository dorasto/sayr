...
import { IconArrowLeft, IconLayoutSidebar, IconLayoutSidebarFilled, IconUsers } from "@tabler/icons-react";
					...
					{organizations
						.flatMap((org) => [
							<SidebarMenu className="gap-0.5 pb-3 last:pb-0" key={org.id}>
								<SidebarMenuItem className="min-h-auto" isActive={pathname === `/settings/org/${org.id}`}>
									<Link to={`/settings/org/$orgId`} params={{ orgId: org.id }} className="w-full">
										<SidebarMenuButton
											size="small"
											icon={
								...
								<SidebarGroup className="ml-2 pl-2 border-l">
									{orgSubItems.map((item) => {
										...
										const isActive =
											item.matchType === "includes" ? pathname.includes(url) : pathname === url;
