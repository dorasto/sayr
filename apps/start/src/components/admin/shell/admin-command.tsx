...
function renderCommandItem(
	...
	return (
		...
		>
			...
			{item.metadata && <span className="ml-auto text-xs text-muted-foreground shrink-0">{item.metadata}</span>}
	...
	const assignmentItems: CommandItemType[] = React.useMemo(() => {
		...
		return assignmentResults.map((task) => {
			...
			return {
				...
				metadata: assignToggleMeta(task.shortId, isAssigned, taskAssignmentContext.orgShortId),
