...
function computeOrgUsdCost(summary: ConsoleOrgAiSummary): number {
...
function buildColumns(
	...
	return [
		...
		},
		{
			...
			accessorKey: "plan",
			cell: ({ row }) => (
				<Badge variant={row.original.plan === "pro" ? "default" : "outline"} className="capitalize">
		...
		},
		{
			...
			cell: ({ row }) => {
				...
				return <span>€{(mrr.mrr_cents / 100).toFixed(2)}/mo</span>;
			},
			size: 100,
			enableHiding: true,
			enableSorting: true,
		...
		},
		{
			header: "Short ID",
			accessorKey: "shortId",
			cell: ({ row }) => <span className="font-mono text-xs text-muted-foreground">{row.original.shortId}</span>,
			size: 100,
			enableHiding: true,
		},
		{
		...
		},
		{
			...
			cell: ({ row }) => {
				const summary = aiSummaryMap.get(row.original.id);
				if (!summary) return <span className="text-muted-foreground">—</span>;
				return <span className="">{formatTokenCount(summary.total_tokens)}</span>;
			},
			size: 120,
			enableHiding: true,
			enableSorting: true,
		},
		{
			...
			accessorFn: (row) => {
				...
				return s ? computeOrgUsdCost(s) : -1;
			},
			sortingFn: "basic",
			cell: ({ row }) => {
				const summary = aiSummaryMap.get(row.original.id);
				if (!summary) return <span className="text-muted-foreground">—</span>;
				const cost = computeOrgUsdCost(summary);
...
export default function OrgTable({ initialData }: OrgTableProps) {
	...
	const [paginationMeta, setPaginationMeta] = useState<ConsolePaginationMeta>(initialData.pagination);
	...
	const [aiSummaryMap, setAiSummaryMap] = useState<Map<string, ConsoleOrgAiSummary>>(new Map());
	useEffect(() => {
	...
	const [mrrSummaryMap, setMrrSummaryMap] = useState<Map<string, ConsoleOrgMrrSummary>>(new Map());
	useEffect(() => {
		...
		<div className="space-y-4">
			...
			<div className="flex flex-wrap items-center justify-between gap-3">
				<div className="flex items-center gap-3">
								...
								<div className="space-y-3">
									{(["free", "pro"] as const).map((value, i) => (
										<div key={value} className="flex items-center gap-2">
											...
											/>
											<Label htmlFor={`${id}-plan-${i}`} className="font-normal capitalize">
												{value}
											</Label>
										</div>
									))}
					...
					{/* Loading indicator */}
					{isLoading && <LoaderIcon size={16} className="animate-spin text-muted-foreground" />}
				</div>
			</div>
