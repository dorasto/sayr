...
export const ConnectionRow = memo(function ConnectionRow({ client, integrations }: ConnectionRowProps) {
	...
	const integration = integrationId ? integrations.find((i) => i.id === integrationId) : undefined;
	return (
		<TableRow>
			{/* User column */}
			<TableCell className="flex items-center gap-2 max-w-[200px]">
				...
				<span className="truncate font-mono text-xs">{client.account?.name || "Unknown"}</span>
					...
					{integration ? (
						<>
							...
							{integration.author?.name && (
								<>
									{" - "}
									<a href={integration.author.url} target="_blank" rel="noopener noreferrer">
										{integration.author.name}
									</a>
								</>
							)}
