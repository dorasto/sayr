...
const SAFE_VALUE_EVENT_TYPES = new Set(["status_change", "priority_change", "category_change", "release_change"]);
			...
			<CardContent className="pt-6">
				<div className="flex items-start gap-4">
					...
					<Avatar className="size-16 rounded-lg">
						...
						<AvatarFallback className="rounded-lg text-lg uppercase">{getInitials(user.name)}</AvatarFallback>
					</Avatar>
					...
					<div className="flex-1 space-y-1">
						<div className="flex items-center gap-2 flex-wrap">
							...
							{!isBanned && isVerified && <Badge variant="secondary">Active</Badge>}
							{!isBanned && !isVerified && <Badge variant="outline">Pending Verification</Badge>}
						</div>
						...
						{isBanned && user.banReason && (
							<div className="text-sm text-destructive">
								...
								{user.banExpires && <span className="ml-2">(expires {formatDateTime(user.banExpires)})</span>}
							</div>
						)}
...
function OrganizationsSection({ organizations }: { organizations: ConsoleUserOrganization[] }) {
					...
					<Card key={org.id}>
						...
						<CardContent className="space-y-3">
							...
							{org.teams.length > 0 ? (
								...
								<div className="text-xs text-muted-foreground">No team assignments</div>
							)}
						</CardContent>
					</Card>
				</Link>
...
function ActivitySection({ activity }: { activity: ConsoleUserDetail["activity"] }) {
			...
			<Card>
				<CardHeader className="pb-3">
					...
					<CardDescription className="text-xs">
						{totalEvents} total event{totalEvents !== 1 ? "s" : ""} across all organizations
					</CardDescription>
				</CardHeader>
				<CardContent>
					{aggregates.length > 0 ? (
						<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
							{aggregates
								...
								.map((agg) => (
									<div key={agg.eventType} className="flex flex-col gap-1 rounded-md border p-3">
...
function RecentEventRow({ event }: { event: ConsoleRecentActivity }) {
	...
	const fromVal = showValues && event.fromValue != null ? String(event.fromValue) : null;
	const toVal = showValues && event.toValue != null ? String(event.toValue) : null;
...
function AccountsSection({ accounts }: { accounts: ConsoleUserAccount[] }) {
	if (accounts.length === 0) {
		return (
			<Card>
				<CardContent className="pt-6">
					<p className="text-sm text-muted-foreground text-center py-4">No connected accounts.</p>
				</CardContent>
			</Card>
		);
	}
							...
							<div key={account.id} className="flex items-center justify-between rounded-md border p-3">
								<div className="flex items-center gap-3">
									...
									<div>
										...
										{account.scope && (
											<div className="text-xs text-muted-foreground">Scopes: {account.scope}</div>
										)}
									</div>
								</div>
