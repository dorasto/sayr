...
export function ConnectionsSnapshotTable({
	...
	const sorted = snapshot.slice().sort((a, b) => (b.connectedAt || 0) - (a.connectedAt || 0));
		...
		<Card>
			...
			<CardContent>
				<Table>
					...
					<TableBody>
						{sorted.map((client) => {
							return <ConnectionRow key={client.sseClientId} client={client} integrations={integrations} />;
						})}
					</TableBody>
				</Table>
			</CardContent>
