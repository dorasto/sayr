...
import { ensureCdnUrl, formatDateTime, formatDateTimeFromNow, getInitials } from "@repo/util";
import {
	IconBan,
	IconCalendar,
	IconCheck,
		...
		<div className="space-y-6">
			...
			<Tabs defaultValue="members" className="w-full flex flex-col shrink-0">
				...
				<TabsContent value="settings" className="mt-4">
					<SettingsSection settings={org.settings as OrganizationSettings | null} />
				</TabsContent>
			...
			<CardContent className="pt-6">
				<div className="flex items-start gap-4">
					<Avatar className="size-16 rounded-lg">
						{org.logo ? <AvatarImage src={ensureCdnUrl(org.logo)} alt={org.name} /> : null}
						<AvatarFallback className="rounded-lg text-lg uppercase">{getInitials(org.name)}</AvatarFallback>
					</Avatar>
					...
					<div className="flex-1 space-y-1">
						<div className="flex items-center gap-2 flex-wrap">
							...
							{isCloud && (
								<Badge variant={org.plan === "pro" ? "default" : "outline"} className="capitalize">
									{org.plan || "free"}
								</Badge>
							)}
						</div>
...
function MembersSection({ members }: { members: ConsoleOrgMember[] }) {
	if (members.length === 0) {
		return (
			<Card>
				<CardContent className="pt-6">
					<p className="text-sm text-muted-foreground text-center py-4">This organization has no members.</p>
				</CardContent>
			</Card>
		);
	}
...
function SettingRow({ label, value }: { label: string; value: boolean | string | null | undefined }) {
...
function SettingsSection({ settings }: { settings: OrganizationSettings | null }) {
	if (!settings) {
		return (
			<Card>
				<CardContent className="pt-6">
	...
	return (
		<div className="space-y-4">
			<Card>
				...
				<CardContent>
					<SettingRow label="Allow Actions on Closed Tasks" value={settings.allowActionsOnClosedTasks} />
					<SettingRow label="Public Actions" value={settings.publicActions} />
					<SettingRow label="Enable Public Page" value={settings.enablePublicPage} />
					<SettingRow label="Public Task Allow Blank" value={settings.publicTaskAllowBlank} />
				</CardContent>
			</Card>
			...
			{settings.publicTaskFields && (
				<Card>
					...
					<CardContent>
						<SettingRow label="Labels" value={settings.publicTaskFields.labels} />
						<SettingRow label="Category" value={settings.publicTaskFields.category} />
						<SettingRow label="Priority" value={settings.publicTaskFields.priority} />
					</CardContent>
				</Card>
			)}
		</div>
	...
	return (
		<Card>
			...
			<CardContent>
				...
				<SettingRow label="Seat Count" value={org.seatCount != null ? String(org.seatCount) : null} />
				<SettingRow label="Polar Customer ID" value={org.polarCustomerId} />
				<SettingRow label="Polar Subscription ID" value={org.polarSubscriptionId} />
				<SettingRow
					label="Current Period End"
					value={org.currentPeriodEnd ? formatDateTime(org.currentPeriodEnd) : null}
				/>
			</CardContent>
		</Card>
	);
