...
export function StatusBar({
  ...
  // --- Grace period: suppress "Connecting" for 1500ms to avoid flicker on navigation ---
  ...
  // Effective status: hide "Connecting" until grace period elapses
  const effectiveStatus = wsStatus === "Connecting" && !showConnecting ? "Connected" : wsStatus;
  ...
  const ws = states[effectiveStatus as keyof typeof states] ?? states.Disconnected;
  ...
  if (effectiveStatus === "Connected" && !children) {
    return null;
  }
