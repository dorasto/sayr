...
import { assignToggleMeta, taskStatusIcon } from "@/lib/command-item-helpers";
import type {
...
export default function AdminCommand() {
  ...
  const taskAssignmentContext = useStore(
    commandStore,
    (state) => state.taskAssignmentContext,
  );
  ...
  const isAssignmentMode =
    !!taskAssignmentContext && activeView === taskAssignmentContext.viewId;

  // Assignment search state
  const [assignmentResults, setAssignmentResults] = React.useState<
    OrgTaskSearchResult[]
  >([]);
  const [isAssignmentSearching, setIsAssignmentSearching] =
    ...
    <>
      ...
      >
        ...
        <CommandInput
          ...
          badge={
            viewBadge ? (
              <span className="mr-2 shrink-0 rounded-lg bg-muted px-2 py-0.5 text-xs font-medium text-muted-foreground">
                {viewBadge}
              </span>
            ) : undefined
          }
            ...
            <>
              ...
              {!search && activeGroups.length > 0 && (
                <>
                  ...
                  {activeGroups.map(
                    (group: CommandGroupType, groupIndex: number) => (
