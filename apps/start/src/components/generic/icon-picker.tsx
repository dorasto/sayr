...
export default function IconPicker({ value, update }: Props) {
  const [icons, setIcons] = useState(
    iconsAll.filter((e) => e.value.includes("Brand")).slice(0, 50),
  );
        ...
        <InputGroup className="h-auto bg-accent border-transparent">
          ...
          <InputGroupAddon align="inline-end">
            <a href="https://tabler.io/icons?utm_source=sayr.io">
              <Button
                className="p-1 h-auto bg-[#1E69C3] hover:bg-[#1E69C3]/80 text-xs"
                size={"sm"}
              >
          ...
          <div className="grid grid-cols-2 gap-3 py-3 md:grid-cols-4">
            ...
            {icons.map((icon, index) => (
              <Button
                ...
                key={index}
                className={cn(
                  "icons-tooltip text-light-copy size-16 text-copy rounded-xl border-2 bg-accent text-3xl [&_svg]:size-auto",
