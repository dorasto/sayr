...
interface ReleaseInfoProps {
	...
	onUpdate: (data: { name: string; slug: string; icon: string; color: string }) => Promise<void>;
	onStatusUpdate: (status: schema.releaseType["status"]) => Promise<void>;
	onTargetDateUpdate: (date: Date | null) => Promise<void>;
}

export function ReleaseInfo({ release, onUpdate, onStatusUpdate, onTargetDateUpdate }: ReleaseInfoProps) {
	...
	return (
		<div className="flex flex-col gap-3">
						...
						<InputGroupButton asChild>
							<Popover modal>
								...
								<PopoverContent className="p-0 w-64 md:w-96">
									<div className="flex flex-col gap-3">
										<div className="p-3">
											<ColorPickerCustom onChange={setEditColor} defaultValue={editColor.hex} height={100} />
										</div>
										<div className="px-3">
											<IconPicker
												value={editIcon}
			...
			{/* Slug */}
			<div className="flex flex-col gap-1.5">
				<Label className="text-xs text-muted-foreground">Release Slug</Label>
				<InputGroup>
					<InputGroupInput
