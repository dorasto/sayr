...
export function ReleasesKanbanCard({ release, orgId }: ReleasesKanbanCardProps) {
		...
		>
			...
			>
				...
				<div className="flex items-start gap-2 w-full min-w-0">
					...
					<Label variant={"subheading"} className="leading-tight line-clamp-1 cursor-pointer max-w-4/5">
						{release.name}
					</Label>
					<Label variant={"description"} className="leading-tight truncate ml-auto cursor-pointer max-w-1/5">
						{release.slug}
					</Label>
				</div>
				...
				{/* Description preview */}
				{descriptionPreview && (
					<Label variant={"description"} className="line-clamp-2 cursor-pointer">
						{descriptionPreview}
					</Label>
				)}
				...
				<div className="flex items-center gap-2 flex-wrap mt-auto pt-1">
					...
					>
						...
						{statusConfig.label}{" "}
						{release.status === "released" && release.releasedAt && formatDate(release.releasedAt)}
					</Badge>
				</div>
			</div>
		</Link>
