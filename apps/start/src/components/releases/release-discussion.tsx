...
import { IconChevronDown, IconDots, IconLoader2, IconMessage, IconPencil, IconTrash } from "@tabler/icons-react";
import { lazy, Suspense, useCallback, useEffect, useMemo, useRef, useState } from "react";
import { CommentInput } from "@/components/shared/comments/comment-input";
import { CommentItem } from "@/components/shared/comments/comment-item";
import { ReplyThreadTrigger } from "@/components/shared/comments/reply-thread-trigger";
...
function applyOptimisticReaction(
	...
	return {
		...comment,
		reactions: total > 0 ? { total, reactions: newReactions } : { total: 0, reactions: {} },
	};
}
...
function applyOptimisticCommentCreate(
	...
	if (newComment.parentId) {
		return [
			...list.map((c) => (c.id === newComment.parentId ? { ...c, replyCount: (c.replyCount ?? 0) + 1 } : c)),
			newComment,
		];
	}
...
interface TopLevelCommentCardProps {
	...
	onPostReply: (parentId: string, content: schema.NodeJSON, visibility: "public" | "internal") => Promise<boolean>;
	onEditReply: (replyId: string, content: schema.NodeJSON) => Promise<boolean>;
	onDeleteReply: (replyId: string) => Promise<boolean>;
	onEditComment: (commentId: string, content: schema.NodeJSON) => Promise<boolean>;
...
function TopLevelCommentCard({
	...
	const [replyInputVisibility, setReplyInputVisibility] = useState<"public" | "internal">("public");
	...
	const replyAuthors: schema.UserSummary[] = Array.from(
		new Map(
			replies.map((r) => [r.createdBy?.id, r.createdBy]).filter((e): e is [string, schema.UserSummary] => !!e[1])
		).values()
	).slice(0, 3);
		...
		<>
			<div
				className={cn(
					...
					comment.visibility === "internal" && "bg-internal border-internal-border"
				)}
			>
				{/* Header + content */}
				<div className="pt-2">
							...
							<>
								...
								<div
									className={cn(
										...
										replies.some((r) => r.visibility === "internal") && "border-internal-border"
									)}
								>
									{replies.map((reply, index) => (
										<div
											...
											className={cn(
												"px-3",
												reply.visibility === "internal" ? "bg-internal" : "bg-card",
												index < replies.length - 1 && "border-b border-border",
												index < replies.length - 1 && reply.visibility === "internal"
...
export function ReleaseDiscussion({
	...
	const [allComments, setAllComments] = useState<schema.ReleaseCommentWithAuthor[]>([]);
	...
	const [mainInputVisibility, setMainInputVisibility] = useState<"public" | "internal">("public");
	...
	const [repliesCache, setRepliesCache] = useState<Map<string, schema.ReleaseCommentWithAuthor[]>>(new Map());
	...
	const loadInitial = useCallback(async () => {
		...
		try {
			// First page (oldest)
			const firstResult = await getReleaseCommentsAction(orgId, releaseId, null, {
				limit: PAGE_SIZE,
				page: 1,
				direction: "asc",
			});
			...
			const unique = Array.from(new Map(merged.map((c) => [c.id, c])).values()).sort(
			...
			if (tp > 1) {
				const lastResult = await getReleaseCommentsAction(orgId, releaseId, null, {
					limit: PAGE_SIZE,
					page: tp,
					direction: "asc",
				});
				if (lastResult.success) {
					...
					const allUnique = Array.from(new Map(allMerged.map((c) => [c.id, c])).values()).sort(
						(a, b) => new Date(a.createdAt ?? 0).getTime() - new Date(b.createdAt ?? 0).getTime()
					);
	...
	useEffect(() => {
		if (prevRefreshKey.current !== null && prevRefreshKey.current === refreshKey) return;
		...
		try {
			...
			if (startResult.success || endResult.success) {
				...
				setAllComments((prev) => {
					...
					const merged = [...prev, ...newComments.filter((c) => !existing.has(c.id))];
					return merged.sort(
						(a, b) => new Date(a.createdAt ?? 0).getTime() - new Date(b.createdAt ?? 0).getTime()
					);
				});
	...
	const applyReaction = useCallback((commentId: string, emoji: ReactionEmoji, userId: string) => {
		setAllComments((list) => list.map((c) => applyOptimisticReaction(c, commentId, emoji, userId)));
	}, []);
	...
	const handlePostComment = useCallback(
		async (content: schema.NodeJSON, visibility: "public" | "internal") => {
			const updatedContent = await processUploads(content, visibility, orgId, "create-release-comment");
			...
			if (result.success && result.data && currentUserSummary) {
				...
				setAllComments((list) => applyOptimisticCommentCreate(list, optimisticComment));
			}
	...
	const handlePostReply = useCallback(
		async (parentId: string, content: schema.NodeJSON, visibility: "public" | "internal") => {
			const updatedContent = await processUploads(content, visibility, orgId, "create-release-comment");
			const result = await createReleaseCommentAction(
				orgId,
				releaseId,
			...
			if (result.success && result.data && currentUserSummary) {
				...
				setAllComments((list) =>
					list.map((c) => (c.id === parentId ? { ...c, replyCount: (c.replyCount ?? 0) + 1 } : c))
				);
			}
			return result.success;
		},
				...
				setRepliesCache((prev) => {
					...
					for (const [parentId, replies] of next) {
						const updated = replies.map((r) =>
							r.id === replyId ? { ...r, content: result.data.content as schema.NodeJSON } : r
						);
	...
	const handleDeleteReply = useCallback(
		async (replyId: string) => {
			...
			if (result.success) {
				...
				if (parentId) {
					setAllComments((list) =>
						list.map((c) => (c.id === parentId ? { ...c, replyCount: Math.max(0, (c.replyCount ?? 0) - 1) } : c))
					);
				}
			}
			return result.success;
	...
	const handleReactionToggle = useCallback(
		async (commentId: string, emoji: ReactionEmoji) => {
			...
			setRepliesCache((prev) => {
				const next = new Map(prev);
				for (const [parentId, replies] of next) {
					const updated = replies.map((r) => applyOptimisticReaction(r, commentId, emoji, currentUserId));
					next.set(parentId, updated);
				}
				return next;
			});
			...
			const hasReacted = target?.reactions?.reactions?.[emoji]?.users.includes(currentUserId) ?? false;

			try {
				if (hasReacted) {
					await removeReleaseCommentReactionAction(releaseId, commentId, emoji);
				} else {
					await addReleaseCommentReactionAction(orgId, releaseId, commentId, emoji, sseClientId);
				}
			} catch {
				// Revert on failure
				applyReaction(commentId, emoji, currentUserId);
	...
	const loadReplies = useCallback(
		async (commentId: string) => {
			...
			try {
				const result = await getReleaseCommentRepliesAction(orgId, releaseId, commentId);
				if (result.success) {
	...
	return (
		<div className="flex flex-col gap-3">
			...
			{loading ? (
				...
				<p className="text-xs text-muted-foreground py-2">No comments yet. Be the first to start the discussion.</p>
			) : (
