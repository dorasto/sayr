...
import { IconArchive, IconChecklist, IconProgress, IconRocket } from "@tabler/icons-react";

export const releaseStatusConfig = {
	planned: {
		label: "Planned",
		icon: (className: string) => <IconChecklist className={className} />,
		className: "[&_svg]:text-blue-500",
		color: "#3B82F6",
	...
	"in-progress": {
		...
		badgeClassName: "bg-yellow-500/10 text-yellow-500 border-yellow-500/20 hover:bg-yellow-500/20",
	},
	released: {
		label: "Released",
	...
	archived: {
		...
		badgeClassName: "bg-gray-500/10 text-gray-500 border-gray-500/20 hover:bg-gray-500/20",
	},
} as const;

export type ReleaseStatusKey = keyof typeof releaseStatusConfig;

export const RELEASE_STATUS_ORDER: Array<ReleaseStatusKey> = ["planned", "in-progress", "released", "archived"];

export const getReleaseStatusConfig = (status: schema.releaseType["status"]) => {
