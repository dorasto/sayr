...
import { IconDots, IconLoader2, IconLock, IconMessage, IconPencil, IconTrash } from "@tabler/icons-react";
...
import { type Health, healthConfig, type Visibility } from "./types";
...
export function StatusUpdateCard({
	...
	const handleSaveEdit = useCallback(
		async (data: { content: schema.NodeJSON | undefined; health: Health; visibility: Visibility }) => {
	...
	const handleEditComment = useCallback(
		async (commentId: string, content: schema.NodeJSON) => {
			...
			if (result.success && result.data) {
				setComments((prev) =>
					prev.map((c) => (c.id === commentId ? { ...c, content: result.data.content as schema.NodeJSON } : c))
				);
			}
	...
	const handleReactionToggle = useCallback(
		async (commentId: string, emoji: ReactionEmoji) => {
			...
			// Optimistic update
			setComments((prev) => prev.map((c) => applyOptimisticReaction(c, commentId, emoji, currentUserId)));
		...
		[currentUserId, comments, applyOptimisticReaction, orgId, releaseId, sseClientId]
	);
		...
		<>
			<div className="rounded-xl border bg-accent relative overflow-hidden p-2 group/update flex flex-col gap-3">
				...
				<div className="flex items-center gap-3">
					<ReadonlyBadges health={health} visibility={update.visibility as Visibility} />
					{update.createdAt && (
						<Label variant="description" className="">
							{formatDateTimeFromNow(update.createdAt)} ({formatDate(update.createdAt)})
						</Label>
					)}
				...
				{!isEditing && commentsOpen && (
					<div className={cn("", update.visibility === "internal" ? "border-primary/20" : "border-border")}>
						{commentsLoading ? (
							...
							<div
								className={cn(
									...
									comments.some((c) => c.visibility === "internal") && "border-internal-border"
								)}
							>
						...
						<CommentInput availableUsers={availableUsers} onPost={handlePostComment} />
					</div>
				)}
			</div>
...
function ReadonlyBadges({ health, visibility }: { health: (typeof healthConfig)[Health]; visibility: Visibility }) {
	return (
		<>
			<Badge variant="outline" className={cn("gap-1 text-xs border pointer-events-none w-fit", health.className)}>
