...
import { ReleasesKanbanCard } from "./releases-kanban-card";
...
export function ReleasesKanban({ releases, setReleases, orgId }: ReleasesKanbanProps) {
	const { runWithToast } = useToastAction();
	const { value: sseClientId } = useStateManagement<string>("sse-clientId", "");

	// Build GridBoard columns from status order
	const gridColumns: GridBoardColumnData[] = RELEASE_STATUS_ORDER.map((status) => {
	...
	const getItemsForCell = useCallback(
		(columnId: string): GridRelease[] => {
			...
			return releases.filter((r) => r.status === status).map((r) => ({ ...r, columnId }));
		},
		[releases]
	);
	...
	const handleDragEnd = async (event: GridBoardDragEndEvent<GridRelease>) => {
		...
		// Optimistic update
		const updated = releases.map((r) => (r.id === item.id ? { ...r, status: newStatus, ...releasedAtUpdate } : r));
		setReleases(updated);
		...
		await runWithToast(
			...
			() => updateReleaseAction(orgId, item.id, { status: newStatus, ...releasedAtUpdate }, sseClientId)
		);
	};
