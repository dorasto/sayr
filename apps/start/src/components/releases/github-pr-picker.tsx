...
export default function GithubPRPicker({
	...
	const [repositories, setRepositories] = useState<Record<number, { id: string; installationId: string }>>({});

	// Fetch repositories for the organization
	useEffect(() => {
		async function fetchRepositories() {
			try {
				const response = await fetch(`${API_URL}/v1/admin/organization/${organizationId}/connections/github`, {
					credentials: "include",
				});
				if (response.ok) {
					const data = await response.json();
					if (data.success) {
						const repoMap: Record<number, { id: string; installationId: string }> = {};
		...
		async (signal?: AbortSignal) => {
			...
			try {
				...
				if (!signal?.aborted && result.success) {
					...
					setResults(result.data.filter((pr: GithubPR) => pr.number !== linkedPRNumber));
				}
			} catch (error) {
	...
	const handleLinkPR = useCallback(
		...
		[organizationId, releaseId, sseClientId, runWithToast, onLinkPR, repositories, onOpenChange]
	);
				...
				<ComboBoxContent shouldFilter={false} className="">
					...
					<ComboBoxList>
						{isLoading ? (
							...
							<ComboBoxEmpty>{query.length > 0 ? "No PRs found" : "No GitHub PRs available"}</ComboBoxEmpty>
						) : (
							<ComboBoxGroup>
								{filteredResults.map((pr) => (
									<ComboBoxItem key={pr.id} value={pr.node_id} onSelect={() => handleLinkPR(pr)}>
										<div className="flex items-center gap-3 p-1">
											<Avatar className="h-6 w-6">
												<AvatarImage src={pr.user.avatar_url} alt={pr.user.login} />
												<AvatarFallback>{getInitials(pr.user.login)}</AvatarFallback>
											</Avatar>
											<div className="flex-1 min-w-0">
												<div className="flex items-center gap-2">
													<span className="text-sm text-muted-foreground">{pr.head.repo.name}</span>
													<span className="font-medium text-sm truncate">#{pr.number}</span>
													<span className="text-xs text-muted-foreground truncate">{pr.title}</span>
												</div>
