...
import { ensureCdnUrl, formatDate, getDisplayName, getInitials, parseGithubPrUrl } from "@repo/util";
import {
	IconCalendarCheck,
	IconCalendarEvent,
	IconCalendarStats,
...
import { InlineCreateLabelForm, RenderLabel } from "@/components/tasks/shared/label";
...
import { addReleaseLabelAction, removeReleaseLabelAction, updateReleaseAction } from "@/lib/fetches/release";
...
import { type ReleaseStatusKey, releaseStatusConfig } from "./config";
import GithubPRPicker from "./github-pr-picker";

type ReleaseFieldKey = "status" | "targetDate" | "releasedAt" | "tasks" | "publicPage" | "lead" | "labels" | "githubPR";
...
const DEFAULT_FIELDS: ReleaseFieldKey[] = ["status", "targetDate", "releasedAt", "githubPR"];
...
export function ReleaseFieldToolbar({
	...
	const canCreateLabel = permissions?.admin?.administrator === true || permissions?.content?.manageLabels === true;
	...
	const handleLeadChange = useCallback(
		async (memberId: string | null) => {
			...
			const member = memberId ? (organization.members.find((m) => m.user.id === memberId)?.user ?? null) : null;
			await runWithToast(
				"update-release-lead",
				{
		...
		return (
			<div className="flex flex-col gap-3">
				...
				{showField("publicPage") && (
					<a href={`https://${organization.slug}.${import.meta.env.VITE_ROOT_DOMAIN}/releases/${release.slug}`}>
						<Button
							variant="primary"
							size="sm"
							className={cn(
					...
					<div className="flex flex-col gap-1.5">
						{editable ? (
							<div className="flex flex-wrap gap-1">
								{currentLabels.map((l) => (
									<RenderLabel
										...
										showRemove={editable}
										onRemove={(id) =>
											handleLabelToggle(currentLabels.map((x) => x.id).filter((x) => x !== id))
										}
									/>
								))}
								<ComboBox values={currentLabels.map((l) => l.id)} onValuesChange={handleLabelToggle}>
									...
									<ComboBoxContent>
										<ComboBoxSearch placeholder="Search labels..." onValueChange={setLabelSearch} />
										<ComboBoxList>
												...
												{canCreateLabel && labelSearch.trim().length > 0 ? (
													<InlineCreateLabelForm
														...
														onCreated={(newLabels) => {
															...
															if (newLabel) {
																handleLabelToggle([...currentLabels.map((l) => l.id), newLabel.id]);
															}
														}}
													/>
												) : (
							...
							<span
								className={cn(
									"text-xs flex items-center gap-1.5",
									currentLabels.length > 0 ? "text-foreground" : "text-muted-foreground"
								)}
							>
							...
							{editable ? (
								<div className="relative group/pr-link flex items-center gap-1 w-full">
									<GithubPRPicker
										...
										disabled={(releaseWithTasks?.githubPullRequests?.length ?? 0) >= 1}
										customTrigger={(() => {
											const linkedPR = releaseWithTasks?.githubPullRequests?.[0];
											const parsed = linkedPR?.prUrl ? parseGithubPrUrl(linkedPR.prUrl) : null;
											return (
												<Button
													variant="primary"
													size="sm"
												...
												>
													...
													<Badge
														...
														className={cn(
															...
															linkedPR?.state === "open" ? "text-success" : "text-muted-foreground",
															!linkedPR && "hidden"
														)}
													>
														{linkedPR?.state}
													...
													{linkedPR?.merged && (
														<Badge variant="secondary" className="h-4 text-xs text-primary">
															Merged
														</Badge>
													)}
												</Button>
									...
									{releaseWithTasks?.githubPullRequests?.[0]?.prUrl && (
										...
										>
											<Button variant="ghost" size="sm" className="h-[26px] aspect-square p-0 rounded-xl">
								...
								<span
									className={cn(
										"text-xs flex items-center gap-1.5",
										releaseWithTasks?.githubPullRequests?.[0] ? "text-foreground" : "text-muted-foreground"
									)}
								>
									<IconGitPullRequest className="w-3 h-3" />
									{(() => {
			...
			{showField("tasks") && (
				...
				>
					...
					<ComboBoxContent shouldFilter={false}>
						<ComboBoxSearch icon placeholder="Search tasks..." onValueChange={handleTaskSearch} />
						<ComboBoxList>
							{taskSearchLoading ? (
								...
								<ComboBoxEmpty>{taskSearch.length > 0 ? "No tasks found" : "No recent tasks"}</ComboBoxEmpty>
							) : (
								<ComboBoxGroup>
									{selectedTasks.map((t) => (
