...
import { IconGitBranch, IconGitPullRequest, IconX } from "@tabler/icons-react";
	...
	return (
		<div className="flex items-center gap-2 p-2 rounded-lg border hover:bg-accent transition-colors w-full">
			<a
				...
				className={`min-w-0 ${
					!editable ? "flex-1 cursor-pointer" : "flex-1 cursor-not-allowed pointer-events-none"
				}`}
			...
			>
				<div className="flex items-center gap-2">
					...
					<div className="flex-1 min-w-0">
						<div className="flex items-center gap-2">
							{repoName && <span className="text-sm text-muted-foreground">{repoName}</span>}
