...
export function ActiveViewSwitcher() {
  ...
  const activeView = personalViews.find(
    (view) => (view.slug || view.id) === viewSlug,
  );
  const isDirty =
    !!activeView &&
    !areStatesEqual({ filters, viewConfig }, getViewCombinedState(activeView));
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="w-56">
        ...
        {isDirty && activeView && (
          <>
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => resetToSavedView(activeView)}>
              Reset changes
            </DropdownMenuItem>
