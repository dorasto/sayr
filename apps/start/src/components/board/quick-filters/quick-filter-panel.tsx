...
const QUICK_FILTER_FIELDS: FilterField[] = [
  "label",
  "org",
  "category",
  "assignee",
...
export function QuickFilterPanel() {
  ...
  const [activeField, setActiveField] = useState<FilterField>(
    QUICK_FILTER_FIELDS[0] as FilterField,
  );
  ...
  const counts = useMemo(
    () => buildFieldValueCounts(baseTasks, activeField),
    [baseTasks, activeField],
  );
  ...
  const options = useMemo<CountedOption[]>(() => {
    ...
    return config
      ...
      .map((option) => ({
        ...option,
        count: (option.mergedValues ?? [option.value]).reduce(
          (sum, v) => sum + (counts.get(v) ?? 0),
          0,
        ),
      }))
  ...
  const activeCondition = filters.groups
    .flatMap((group) => group.conditions)
    .find((c) => c.field === activeField);
  ...
  const handleToggle = (option: CountedOption) => {
    ...
    setFilters(
      toggleFieldValues(
        filters,
        activeField,
        config.filterDefault,
        option.mergedValues ?? [option.value],
      ),
    );
  };
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex max-h-64 flex-col gap-0.5 overflow-y-auto">
        ...
        {options.map((option) => {
          const isSelected = (option.mergedValues ?? [option.value]).every(
