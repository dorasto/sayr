...
import { TASK_GROUPING_OPTIONS, TASK_GROUPINGS } from "../config/grouping-options";
...
export function BoardViewOptions() {
	...
	const subGroupingOptions = useMemo(
		() => [
			{ id: "none" as const, label: "None", icon: <IconEyeOff className="h-4 w-4" /> },
				...
				<OptionField
					...
					customSide={
						<DropdownMenu>
							<DropdownMenuTrigger
								render={
									<Button type="button" variant="accent" className={cn("gap-2 border-transparent p-1 h-auto")}>
										{activeGrouping.icon}
