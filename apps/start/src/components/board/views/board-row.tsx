...
export function BoardRow({ task, nested = false }: BoardRowProps) {
  ...
  return (
    <Link
      ...
      className="group flex items-center gap-1.5 px-2 py-1 text-xs hover:bg-accent transition-all rounded-xl"
    >
      ...
      {nested && (
        <span
          ...
          className={cn(
            ROW_LEADING_GUTTER_CLASS,
            "h-3.5 shrink-0 flex items-center justify-center",
          )}
        >
      ...
      {task.organization ? (
        <OrgHoverCard organization={task.organization}>
          ...
          >
            <Avatar className="size-3.5 rounded-sm shrink-0">
              <AvatarImage
                src={task.organization.logo ?? undefined}
        ...
        <span
          className={cn(
            "shrink-0 text-xs font-medium text-muted-foreground truncate",
            ORG_KEY_SLOT_CLASS,
          )}
        >
          {task.shortId}
      ...
      >
        {task.visible === "private" && (
          <IconLock className="size-3.5 mr-1 text-primary shrink-0" />
        )}
        {task.title || "Untitled"}
      </span>
      {/*
