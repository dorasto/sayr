...
export default function Editor({
  ...
  const mentionedUserIds = useMemo(
    () => extractMentionUserIds(defaultContent),
    [defaultContent],
  );
      ...
      <ProseKit editor={editor}>
        <div className={className}>
          <div
            ref={editor.mount}
            className={cn(
              'ProseMirror box-border min-h-full px-12 py-0! outline-none outline-0 [&_span[data-mention="tag"]]:text-violet-500 prose',
          ...
          {!readonly && (
            <>
              ...
              {!hasTemplate ? (
                <SlashMenu
                  onOpenChange={(open) => handleMenuOpenChange("slash", open)}
                />
              ) : (
                <SlashMenuTemplate
                  onOpenChange={(open) => handleMenuOpenChange("slash", open)}
