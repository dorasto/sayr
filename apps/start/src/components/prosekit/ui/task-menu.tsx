...
export default function TaskMenu(props: {
	...
	const handleTagInsert = (id: string, taskShortId: number) => {
		editor.commands.insertMention({
			id: id.toString(),
			value: props.orgShortId ? formatTaskKey(props.orgShortId, taskShortId) : `#${taskShortId}`,
			kind: "task",
		});
	};
			...
			<AutocompleteList filter={null}>
				...
				{!props.loading &&
					props.tasks.map((task) => {
						...
						const formattedKey = props.orgShortId
							? formatTaskKey(props.orgShortId, task.shortId as number)
							: `#${shortId}`;
						...
						return (
							...
							>
								<InlineLabel
									...
									text={`${formattedKey} ${task.title ?? ""}`}
									textNode={
										<>
											<span className="text-muted-foreground! text-xs!">{formattedKey}</span>
