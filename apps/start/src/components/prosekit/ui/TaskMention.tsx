...
export function TaskMention({ task, categories, hide }: TaskMentionProps) {
	...
	const orgShortId = "organization" in task ? task.organization?.shortId : undefined;
	// Route param needs the raw numeric shortId — displayKey is the org-prefixed
		...
		<Tooltip>
			...
			>
				<Link to="/$orgId/tasks/$taskShortId" params={{ orgId: task.organizationId, taskShortId: routeShortId }}>
					...
					<Badge
						variant="outline"
						className={cn("inline-flex items-center gap-1 justify-start text-xs mb-1 border-transparent")}
					>
						{statusIcon}
