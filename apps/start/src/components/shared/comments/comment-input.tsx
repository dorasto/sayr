...
export function CommentInput({
		...
		<ButtonGroup>
			{submitLabel ? (
				<Button
					...
					className={cn(
						"border-0 h-7",
						showVisibilityToggle && visibility === "internal" && "bg-internal/50 hover:bg-internal"
					)}
				>
	...
	return (
		<div className={cn("text-foreground transition-all flex gap-2 items-start p-2", className)}>
			...
			<div className={cn("flex-1 min-w-0", !multiline && "flex items-center gap-2")}>
				...
				{multiline ? <div className="flex items-center justify-end mt-1">{actionButtons}</div> : actionButtons}
			</div>
		</div>
	);
}
