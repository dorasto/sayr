...
import { IconCheck, IconDots, IconLoader2, IconLock, IconPencil, IconTrash, IconX } from "@tabler/icons-react";
...
export function CommentItem({
	...
	const [editContent, setEditContent] = useState<schema.NodeJSON | undefined>(comment.content ?? undefined);
		...
		<>
			<div
				className={cn(
					...
					comment.visibility === "internal" && "bg-internal px-1 border border-internal-border",
					className
				)}
			>
				...
				{isEditing ? (
					<>
						<Suspense fallback={<div className="h-10 animate-pulse bg-muted rounded" />}>
							<Editor defaultContent={comment.content ?? undefined} onChange={setEditContent} hideBlockHandle />
						</Suspense>
					...
					comment.content && (
						<Suspense fallback={<div className="h-4 animate-pulse bg-muted rounded w-3/4" />}>
							<Editor readonly defaultContent={comment.content} hideBlockHandle className="pl-1" />
						</Suspense>
					)
				)}

				{/* Reaction display — shown when reactions exist, below content */}
				{!isEditing && onReactionToggle && (comment.reactions?.total ?? 0) > 0 && (
					<div className="pb-3">
						<ReactionDisplay
