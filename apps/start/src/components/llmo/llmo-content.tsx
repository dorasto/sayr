...
function CommentThread({ comment, depth = 0 }: { comment: LLMOComment; depth?: number }) {
	return (
...
export function LLMOContent({
	...
	// `shortId` arrives pre-formatted via `formatTaskKey` (e.g. "SAY-123") whenever the
	// caller has an org shortId available — don't re-add a "#" prefix on top of it.
	const id = shortId ? String(shortId) : undefined;
	...
	return (
		<div className="sr-only" aria-hidden="true" data-llmo-content>
			<article>
				...
				{priority && priority !== "none" && <p>Priority: {priority}</p>}
				{labels && labels.length > 0 && <p>Labels: {labels.join(", ")}</p>}
