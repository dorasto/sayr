...
import { Tile, TileAction, TileHeader, TileIcon } from "@repo/ui/components/doras-ui/tile";
		...
		<section className="flex flex-col gap-3">
			...
			<div className="flex flex-wrap gap-3 w-full">
				{items.map((invite) => {
					...
					return (
						<Tile key={invite.id} className="flex-[1_1_calc(33.333%-0.5rem)] min-w-full">
							<TileHeader className="w-full">
								...
								<div className="flex-1 min-w-0">
									<p className="font-medium text-sm truncate">{org?.name ?? "Unknown organization"}</p>
							...
							<TileAction>
								...
								>
									{isAccepting ? <IconLoader2 className="size-3.5 animate-spin" /> : "Accept"}
								</Button>
								<Button
									size="sm"
									variant="outline"
								...
								>
									{isDeclining ? <IconLoader2 className="size-3.5 animate-spin" /> : "Decline"}
								</Button>
							</TileAction>
						</Tile>
					);
