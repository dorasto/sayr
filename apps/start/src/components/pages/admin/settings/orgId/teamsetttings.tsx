...
export default function SettingsOrganizationPageTeamSettings({
	...
	const [permissions, setPermissions] = useState<TeamPermissions>(team?.permissions || defaultTeamPermissions);
	...
	const handleRemoveMember = async (memberId: string) => {
		...
		const result = await runWithToast(
			...
			() => removeOrganizationMemberFromTeamAction(organization.id, team.id, memberId)
		);
	...
	const availableMembers = organization.members.filter((m) => !teamMemberIds.has(m.id));

	// Permission update helpers
	const updateAdminPermission = (key: keyof TeamPermissions["admin"], value: boolean) => {
		setPermissions((prev) => ({
			...prev,
	...
	const updateContentPermission = (key: keyof TeamPermissions["content"], value: boolean) => {
		setPermissions((prev) => ({
			...prev,
	...
	const updateTasksPermission = (key: keyof TeamPermissions["tasks"], value: boolean) => {
		setPermissions((prev) => ({
			...prev,
	...
	const updateModerationPermission = (key: keyof TeamPermissions["moderation"], value: boolean) => {
		setPermissions((prev) => ({
			...prev,
		...
		<Tabs defaultValue="settings" className={"w-full min-w-full"}>
			...
			<TabsPanel value="permissions" className="w-full">
				<div className="flex flex-col gap-2">
					...
					<div className={cn("rounded-lg p-4 transition-colors", isAdmin ? "bg-destructive/10" : "bg-card")}>
						<div className="flex items-start justify-between gap-4">
							<div className="flex items-start gap-3">
								<div className={cn("p-2 rounded-lg", isAdmin ? "bg-destructive/20" : "bg-muted")}>
									<IconShield className={cn("size-5", isAdmin && "text-destructive")} />
								</div>
								<div className="flex flex-col gap-1">
									<Label className="font-medium">Administrator</Label>
									<Label variant="description" className="text-sm">
										Full access to all organization settings and content. Overrides all other permissions.
									</Label>
								</div>
							</div>
							<Switch
					...
					{isAdmin && (
						<div className="rounded-lg bg-muted/50 border border-dashed p-3 text-center">
							<Label variant="description" className="text-sm">
								Administrator permission grants full access. Other permissions are ignored.
							</Label>
						</div>
					)}
					...
					<Collapsible
						...
						className={cn("bg-card rounded-lg", isAdmin && "pointer-events-none opacity-50")}
						disabled={isAdmin}
					>
						...
						<CollapsibleContent>
							<div className="pl-4 pr-2 pb-2 flex flex-col gap-2">
								<PermissionRow
									...
									onCheckedChange={(checked) => updateAdminPermission("manageMembers", checked)}
									disabled={isAdmin}
								/>
								<PermissionRow
									label="Manage teams"
						...
						<CollapsibleContent>
							<div className="pl-4 pr-2 pb-2 flex flex-col gap-2">
								<PermissionRow
									...
									onCheckedChange={(checked) => updateContentPermission("manageCategories", checked)}
									disabled={isAdmin}
								/>
								<PermissionRow
									label="Manage labels"
					...
					>
						<CollapsibleTrigger asChild>
							...
							>
								<div className="flex items-center gap-2">
									...
									<Badge variant="secondary" className="text-xs">
										{Object.values(permissions.tasks).filter(Boolean).length}/6
									</Badge>
								</div>
								<IconChevronDown
									className={cn(
						...
						<CollapsibleContent>
							<div className="pl-4 pr-2 pb-2 flex flex-col gap-3">
								...
								<div className="flex flex-col gap-2">
									<Label variant="description" className="text-xs uppercase tracking-wide">
										Task Actions
									</Label>
									<div className="flex flex-wrap gap-4">
										<CheckboxItem
											...
											onCheckedChange={(checked) => updateTasksPermission("create", checked)}
											disabled={isAdmin}
										/>
										<CheckboxItem
											label="Edit any"
											checked={permissions.tasks.editAny}
											onCheckedChange={(checked) => updateTasksPermission("editAny", checked)}
											disabled={isAdmin}
										/>
										<CheckboxItem
											label="Delete any"
											checked={permissions.tasks.deleteAny}
											onCheckedChange={(checked) => updateTasksPermission("deleteAny", checked)}
											disabled={isAdmin}
										/>
										<CheckboxItem
											label="Assign"
											checked={permissions.tasks.assign}
											onCheckedChange={(checked) => updateTasksPermission("assign", checked)}
											disabled={isAdmin}
										/>
									</div>
								</div>
								...
								<div className="flex flex-col gap-2">
									...
									<div className="flex flex-wrap gap-4">
										<CheckboxItem
											...
											onCheckedChange={(checked) => updateTasksPermission("changeStatus", checked)}
											disabled={isAdmin}
										/>
										<CheckboxItem
											label="Change priority"
											checked={permissions.tasks.changePriority}
											onCheckedChange={(checked) => updateTasksPermission("changePriority", checked)}
											disabled={isAdmin}
										/>
									</div>
								</div>
						...
						<CollapsibleTrigger asChild>
							...
							>
								<div className="flex items-center gap-2">
									...
									<Label className="font-medium cursor-pointer">Moderation</Label>
									<Badge variant="secondary" className="text-xs">
										{Object.values(permissions.moderation).filter(Boolean).length}
										/3
									</Badge>
								</div>
								<IconChevronDown
						...
						<CollapsibleContent>
							<div className="pl-4 pr-2 pb-2 flex flex-col gap-2">
								<PermissionRow
									...
									onCheckedChange={(checked) => updateModerationPermission("manageComments", checked)}
									disabled={isAdmin}
								/>
								<PermissionRow
									label="Approve submissions"
									description="Approve or reject public bug reports and feedback"
									checked={permissions.moderation.approveSubmissions}
									onCheckedChange={(checked) => updateModerationPermission("approveSubmissions", checked)}
									disabled={isAdmin}
								/>
								<PermissionRow
									label="Manage votes"
									description="Reset votes, handle vote fraud"
									checked={permissions.moderation.manageVotes}
									onCheckedChange={(checked) => updateModerationPermission("manageVotes", checked)}
									disabled={isAdmin}
								/>
							</div>
						</CollapsibleContent>
				...
				<TabsPanel value="members" className="w-full">
					<div className="bg-card rounded-lg flex flex-col">
						...
						<Popover open={addMemberOpen} onOpenChange={setAddMemberOpen}>
							<PopoverTrigger asChild>
								<Tile className="md:w-full hover:bg-accent cursor-pointer" variant="transparent">
									<TileHeader className="md:w-full">
										...
										<TileDescription>Add an organization member to this team</TileDescription>
									</TileHeader>
								</Tile>
							</PopoverTrigger>
							<PopoverContent className="p-0 w-80">
								<div className="p-3 border-b">
									...
									<Label variant="description">Select an organization member to add</Label>
								</div>
										...
										availableMembers.map((member) => (
											...
											>
												<TileHeader className="md:w-full">
													...
													<TileTitle className="text-sm">{member.user.name}</TileTitle>
													<TileDescription className="text-xs">{member.user.email}</TileDescription>
												</TileHeader>
											</Tile>
										))
									)}
						...
						{team.members.map((teamMember) => {
							const orgMember = organization.members.find((m) => m.id === teamMember.memberId);
									...
									<DropdownMenuTrigger asChild>
										<Tile className="md:w-full hover:bg-accent cursor-pointer" variant="transparent">
											<TileHeader className="md:w-full">
												...
												<TileDescription>{orgMember.user.email}</TileDescription>
											</TileHeader>
											<TileAction>
						...
						{team.members.length === 0 && (
							<div className="p-6 text-center text-muted-foreground">
								...
								<p className="text-sm">
									No members in this team yet. Add members to grant them team permissions.
								</p>
							</div>
						)}
					</div>
