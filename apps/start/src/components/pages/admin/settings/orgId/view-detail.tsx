...
export default function SettingsOrganizationViewDetailPage({
	...
	const { value: sseClientId } = useStateManagement<string>("ws-clientId", "", 1);
		...
		<div className="flex flex-col gap-3">
			...
			<div className="bg-card rounded-lg flex flex-col">
				...
				<Tile className="md:w-full" variant={"transparent"}>
					...
					<TileAction className="gap-2">
						<DropdownMenu>
							<DropdownMenuTrigger asChild>
								<Button variant="outline" className="gap-2">
									<IconArrowsSort className="h-4 w-4" />
									<span>{TASK_SORT_FIELDS.find((f) => f.id === viewConfig?.sortBy)?.label || "None"}</span>
								</Button>
							</DropdownMenuTrigger>
							<DropdownMenuContent align="end">
								<DropdownMenuRadioGroup
									value={viewConfig?.sortBy || "none"}
									onValueChange={(v) =>
										setViewConfig((prev) => ({
											...(prev ?? defaultConfig),
											sortBy: v as TaskSortField | "none",
										}))
									}
								>
									<DropdownMenuRadioItem value="none">None</DropdownMenuRadioItem>
						...
						{viewConfig?.sortBy && viewConfig.sortBy !== "none" && (
							<Button
								variant="outline"
								size="icon"
								aria-label={
			...
			<div className="flex flex-col gap-4 max-w-2xl">
				{viewsOverLimit && (
					<p className="text-sm text-destructive">
						{getLimitMessage("savedViews")} Editing is disabled until you remove views to get back under the
						limit.
					</p>
				)}
				<div className="flex items-center gap-2 mt-4">
					<Button onClick={handleSave} variant={"primary"} disabled={viewsOverLimit}>
