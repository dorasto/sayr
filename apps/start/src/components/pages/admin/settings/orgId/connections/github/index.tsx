	...
	return (
		<div className="flex flex-col gap-6">
			<InstallationsSection
				...
				isLoading={isLoading}
			/>
			<TaskSyncSection githubConnections={githubConnections} repositories={repositories} isLoading={isLoading} />
		</div>
	);
}
		...
		<div className="flex flex-col gap-3">
			<div className="flex items-start justify-between">
				<div className="flex flex-col">
					...
					<Label variant={"description"}>Linked repositories across all GitHub installations</Label>
				</div>
			...
			<div className="bg-card rounded-lg flex flex-col">
				{repositories.map((repo) => {
					const connection = githubConnections.find((c) => c.installation.installationId === repo.installationId);
								...
								<DropdownMenuItem
									onClick={async () => {
										...
										await runWithToast(
											...
											{
												loading: {
													title: nextState ? "Enabling sync..." : "Disabling sync...",
												},
												success: {
											...
											() => toggleGithubSyncConnectionAction(organization.id, repo.id, nextState)
										);
				...
				{repositories.length === 0 && (
					<div className="p-4 text-sm text-muted-foreground">No repositories linked yet.</div>
				)}
			</div>
			...
			/>
			<AdaptiveDialog open={deleteId !== null} onOpenChange={(open) => !open && setDeleteId(null)}>
				<AdaptiveDialogContent>
							...
							onClick={async () => {
								...
								await runWithToast(
									"delete-github-sync-connection",
									{
										loading: {
											title: "Deleting sync...",
											description: "Please wait while we remove the sync connection.",
										},
										success: {
											title: "Sync removed",
											description: "The sync connection has been successfully removed.",
										},
										error: {
											title: "Failed to remove sync",
											description: "An error occurred while removing the sync connection.",
										},
									},
...
function SyncDialog({
	...
	const usedRepoIds = new Set(
		allSyncedRepos.filter((r) => (isEditing ? r.id !== editingRepo?.id : true)).map((r) => r.repoId)
	);
	...
	const otherRepos = allSyncedRepos.filter((r) => (isEditing ? r.id !== editingRepo?.id : true));
	...
	const availableCategories = categories.filter((c) => !usedCategoryIds.has(c.id) || c.id === editingRepo?.categoryId);
	/* ================= DISPLAY NAME ================= */

	const selectedRepoName = allRepos.find((r) => r.id.toString() === selectedRepo)?.full_name;

	const selectedCategoryData =
		selectedCategory === "__none__" ? { name: "No category" } : categories.find((c) => c.id === selectedCategory);

	const finalCategoryId = selectedCategory === "__none__" ? null : selectedCategory;

	const isUnchanged =
		isEditing && selectedRepo === editingRepo?.repoId?.toString() && finalCategoryId === editingRepo?.categoryId;
	/* ================= UI ================= */
	...
	return (
		<AdaptiveDialog open={open} onOpenChange={onOpenChange}>
			<AdaptiveDialogContent>
				<AdaptiveDialogHeader className="bg-card">
					<AdaptiveDialogTitle>{isEditing ? "Edit sync" : "Add sync"}</AdaptiveDialogTitle>

					<AdaptiveDialogDescription>Link a repository to a category.</AdaptiveDialogDescription>
				</AdaptiveDialogHeader>
								...
								<ComboBoxList>
									...
									<ComboBoxGroup>
										{availableRepos.map((repo) => (
											<ComboBoxItem key={repo.id} value={repo.id.toString()} searchValue={repo.name}>
				...
				<AdaptiveDialogFooter>
					...
					<Button
						...
						onClick={async () => {
							...
							const selectedRepoData = allRepos.find((r) => r.id.toString() === selectedRepo);
							...
							const installation = githubInfos.find((info) =>
								info.repositories.some((repo: { id: any }) => repo.id === repoId)
							);
							...
							if (isEditing) {
								...
								if (repoId === editingRepo.repoId && finalCategoryId === editingRepo.categoryId) {
									onOpenChange(false);
									return;
								}
								...
								const data = await runWithToast(
									"update-github-sync-connection",
									{
										loading: {
											title: "Updating sync...",
											description: "Please wait while we update the sync connection.",
										},
										success: {
											title: "Sync updated",
											description: "The sync connection has been successfully updated.",
										},
										error: {
											title: "Failed to update sync",
											description: "An error occurred while updating the sync connection.",
										},
									},
									() =>
										updateGithubSyncConnectionAction(organization.id, editingRepo.id, {
