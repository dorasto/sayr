...
export default function AiSettingsPage() {
	...
	const { organization, setOrganization, permissions } = useLayoutOrganizationSettings();
	...
	const handleToggle = useCallback(
		async (key: keyof OrgAiSettings, checked: boolean) => {
			...
			const updatedSettings: OrganizationSettings = { ...orgSettings, ai: updatedAi };
			...
			try {
				...
				if (result.success) {
					...
					headlessToast.success({ title: "AI setting updated" });
				} else {
					setOrganization({ ...organization, settings: orgSettings });
					headlessToast.error({ title: result.error || "Failed to update setting" });
				}
			} catch {
				setOrganization({ ...organization, settings: orgSettings });
		...
		<div className="flex flex-col gap-6">
			...
			{!isAdmin && (
				<div className="flex items-center gap-2 rounded-md border border-border bg-muted/50 px-3 py-2.5 text-xs text-muted-foreground">
					...
					<span>Only organization administrators can change these settings. You are viewing them in read-only mode.</span>
				</div>
			)}
				...
				<div className="bg-card rounded-lg flex flex-col">
					<Tile className="md:w-full" variant="transparent">
						<TileHeader className="md:w-full">
							...
							<TileDescription className="text-xs leading-normal!">
								Show an AI-generated summary panel on task detail pages. Members can generate a concise
								overview of the task based on its description and comments.
							</TileDescription>
						</TileHeader>
						<TileAction>
							<Switch
