...
import { Tile, TileDescription, TileTitle } from "@repo/ui/components/doras-ui/tile";
import { Label } from "@repo/ui/components/label";
import { Progress } from "@repo/ui/components/progress";
import { Separator } from "@repo/ui/components/separator";
import { Skeleton } from "@repo/ui/components/skeleton";
import { formatDate as formatDateUtil } from "@repo/util";
...
// Thin null-safe wrapper — @repo/util's formatDate doesn't accept null/undefined,
// but currentPeriodStart/End can be null here, so this preserves the "N/A" fallback.
...
export function BillingSubscriptionDetails({ subscription }: BillingSubscriptionDetailsProps) {
	...
	const pricePerSeat = subscription.seats ? subscription.amount / subscription.seats : subscription.amount;
				...
				<Tile className="md:w-full flex-col items-start gap-2">
					<div className="flex items-center justify-between w-full">
						<div className="flex items-center gap-2">
							...
							<span className="text-sm font-medium text-foreground">{subscription.product.name}</span>
						</div>
						<div className="flex items-center gap-2">
							{isCanceling ? (
					...
					<div className="flex items-baseline gap-1">
						<span className="font-semibold text-foreground">{subscription.seats} seats</span>
						<span className="text-sm text-muted-foreground">
					...
					<div className="flex flex-col gap-1 w-full">
						...
						{!isCanceling && !isCanceled && subscription.currentPeriodEnd && (
							<div className="flex items-center justify-between">
								<span className="text-xs text-muted-foreground">Next due date</span>
								<span className="text-xs text-foreground">{formatDate(subscription.currentPeriodEnd)}</span>
							</div>
						)}
					</div>
					{isCanceling && (
					...
					{isCanceled && (
						<div className="flex items-center gap-1.5">
							<IconAlertTriangle className="size-3.5 text-yellow-500" />
							<span className="text-xs text-yellow-500">Subscription canceled</span>
						</div>
					)}
				</Tile>
				...
				<Tile className="md:w-full flex-col gap-2 items-stretch p-3 py-2">
					<div className="flex items-center justify-between">
						...
						<TileDescription
							className={cn(
								"text-sm",
								assignedCount >= (subscription.seats ?? 0) ? "text-destructive" : "text-foreground"
							)}
						>
					...
					<Progress
						value={
							(subscription.seats ?? 0) > 0 ? Math.round((assignedCount / (subscription.seats ?? 1)) * 100) : 0
						}
						className={cn("h-4", assignedCount >= (subscription.seats ?? 0) && "[&>div]:bg-destructive")}
					/>
					<Alert
						className={cn(
							"border mt-1",
