...
import { IconDots, IconPlus, IconSettings, IconTrash, IconUsers, IconUsersGroup } from "@tabler/icons-react";
		...
		<div className="bg-card rounded-lg flex flex-col">
			...
			<Link to="/settings/org/$orgId/teams/new" params={{ orgId: organization.id }}>
				<Tile className="md:w-full hover:bg-accent data-[state=open]:bg-accent" variant={"transparent"}>
					<TileHeader className="md:w-full">
						...
						<TileDescription>Add a new team to the organization</TileDescription>
					</TileHeader>
				</Tile>
			</Link>
						...
						>
							<Tile className="md:w-full hover:bg-accent data-[state=open]:bg-accent" variant={"transparent"}>
								<TileHeader className="md:w-full">
									...
									<TileDescription>{team.description || "No description"}</TileDescription>
								</TileHeader>
			...
			{teams.length === 0 && (
				<div className="p-6 text-center text-muted-foreground">
					...
					<p className="text-sm">No teams yet. Create a team to organize members and manage permissions.</p>
				</div>
			)}
		</div>
	);
