...
import { IconChevronLeft, IconChevronRight, IconDownload, IconFileInvoice } from "@tabler/icons-react";
import { useCallback, useEffect, useState } from "react";
import { useLayoutOrganizationSettings } from "@/contexts/ContextOrgSettings";
import { getOrderHistory, getOrderInvoice, type OrderEntry } from "@/lib/fetches/organization";
...
function getOrderStatusVariant(status: string): "default" | "outline" | "secondary" | "destructive" {
	switch (status) {
		case "paid":
			return "outline";
		case "pending":
...
export function BillingOrderHistory() {
	...
	if (error) {
		return (
			<div className="flex flex-col gap-3">
				<Label variant="subheading">Invoice History</Label>
				<div className="rounded-lg border p-4 text-sm text-muted-foreground">{error}</div>
			</div>
		);
	}
							...
							<TableRow key={order.id}>
								...
								<TableCell className="text-xs">
									<div className="flex flex-col gap-0.5">
										<span className="text-foreground">{getBillingReasonLabel(order.billingReason)}</span>
										{order.product && (
								...
								<TableCell>
									<Badge
										...
										className={cn("text-xs", order.status === "paid" && "text-green-500")}
									>
								...
								<TableCell>
									{order.isInvoiceGenerated && (
										...
										>
											<IconDownload
												className={cn("size-3.5", downloadingId === order.id && "animate-pulse")}
											/>
										</Button>
									)}
								</TableCell>
