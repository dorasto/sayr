...
import { getSubscriptionDetails, type SubscriptionDetails } from "@/lib/fetches/organization";
...
export default function SettingsOrganizationBillingPage({ limits }: { limits: PlanLimits }) {
	...
	const { organization, setOrganization, views, issueTemplates, releases } = useLayoutOrganizationSettings();
	...
	const memberLimit =
		organization.plan === "pro" ? (subscription?.seats ?? organization.seatCount ?? limits.members) : limits.members;
