...
export default function SettingsOrganizationPageMembers({
	...
	const canManageTeams = useMemo(() => {
		...
		return currentMember.teams.some(
			(mt) => mt.team.permissions.admin.administrator || mt.team.permissions.admin.manageTeams
		);
	...
	const handleToggleTeam = async (memberId: string, teamId: string, isCurrentlyInTeam: boolean) => {
		...
		try {
			if (isCurrentlyInTeam) {
				const result = await runWithToast(
					...
					() => removeOrganizationMemberFromTeamAction(organization.id, teamId, memberId)
				);
				if (result?.success) {
					setOrganization({
						...organization,
				...
				const result = await runWithToast(
					...
					() => addOrganizationMemberToTeamAction(organization.id, teamId, memberId)
				);
				if (result?.success) {
	...
	return (
		<div className="bg-card rounded-lg flex flex-col">
			...
			{mergeMembersAndInvites?.map((member) => {
				const memberTeamIds = new Set("teams" in member && member.teams ? member.teams.map((mt) => mt.teamId) : []);
									...
									<TileTitle>
										...
										{!isInvite && organization.createdBy === member.userId && (
											<Tooltip>
												...
												<TooltipContent>Owner of this organization</TooltipContent>
											</Tooltip>
										)}
										{!isInvite && !hasSeat && (
											<Badge
										...
										{"teams" in member &&
											...
											member.teams.map((mt) => (
												...
												>
													...
													<span className="truncate max-w-24">{mt.team.name}</span>
												</Badge>
											))}
									</div>
						...
						<DropdownMenuContent align="end">
								...
								<DropdownMenuSub>
									...
									<DropdownMenuSubContent>
										...
										{teams.map((team) => {
											...
											const isToggling = togglingTeam === `${member.id}-${team.id}`;
											return (
												<DropdownMenuItem
													key={team.id}
													onSelect={(e) => {
												...
												>
													{isToggling ? (
														...
														<Checkbox checked={isInTeam} className="pointer-events-none" />
													)}
							...
							<DropdownMenuItem
								...
								onClick={async () => {
									...
									if (!confirmed) return;
									const result = await deleteOrganizationMemberAction(organization.id, member.user.id);
									if (result?.success) {
										setOrganization({
											...organization,
											members: organization.members.filter((m) => m.userId !== member.user.id),
										});
									}
								}}
							>
				...
				{organization.members.filter((m) => m.seatAssigned).length >= (organization.seatCount ?? 0) ? (
					<Tile variant="outline" className="md:w-full border-destructive/30 bg-destructive/5">
						<TileHeader>
							...
							<TileDescription>
								You have reached the maximum number of members for your current plan. Please upgrade your plan
