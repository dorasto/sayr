...
import { getInitials, isSlugBanned } from "@repo/util";
...
import { updateOrganizationAction, uploadOrganizationBanner, uploadOrganizationLogo } from "@/lib/fetches/organization";
...
export default function SettingsOrganizationPage() {
	...
	const [, { openFileDialog: openBannerFileDialog, getInputProps: getBannerInputProps, inputRef: bannerInputRef }] =
		useFileUpload({
			accept: "image/*",
			multiple: false,
		});
		...
		<div className="bg-card rounded-lg flex flex-col">
			...
			<Tile className="md:w-full w-full items-start" variant={"transparent"}>
				...
				<TileAction className="w-full flex flex-col">
					<InputGroup
						className={cn(
							"bg-accent border-0 shadow-none transition-all",
							slugError && "ring-1 ring-destructive"
						)}
					>
						<InputGroupInput
							placeholder="My Organization"
			...
			<Tile className="md:w-full w-full items-start" variant="transparent">
				<TileHeader className="w-full">
					...
					<TileDescription asChild>
						<Label variant={"description"} className="text-xs">
							A short, 3-letter code used as a prefix for IDs and branches. For example: DEV-123.
						</Label>
					</TileDescription>
				</TileHeader>
				<TileAction className="w-full">
					<InputGroup className="bg-accent border-0 shadow-none transition-all">
						...
						/>
						<InputGroupAddon align="inline-end">
							<InputGroupText>{orgShortId ? `${orgShortId}-123` : "SAY-123"}</InputGroupText>
							<Separator orientation="vertical" className="h-3" />
							<InputGroupButton
								variant="ghost"
								size="icon-sm"
								...
								className={cn(!shortIdChanged && "opacity-50 text-foreground/0")}
							>
								<IconCheck />
							</InputGroupButton>
						</InputGroupAddon>
					...
					<InputGroup className="bg-accent border-0 shadow-none transition-all">
						...
						<InputGroupAddon align="inline-end">
							<InputGroupButton
								...
								className={cn(!descriptionChanged && "opacity-50 text-foreground/0")}
							>
								<IconCheck />
							</InputGroupButton>
						</InputGroupAddon>
			...
			<input ref={bannerInputRef} {...getBannerInputProps()} onChange={handleBannerFileSelect} />
