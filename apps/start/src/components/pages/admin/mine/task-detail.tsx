...
export function MyTaskDetail({
	...
	const debouncedUpdateLabels = useDebounceAsync(async (values: string[], wsClientId: string) => {
	...
	const fullUrl = "/" + task.organizationId + "/tasks/" + task.shortId?.toString();
		...
		<div className="flex flex-col h-full">
			...
			<div className="p-3 border-b">
				<div className="flex items-start justify-between w-full gap-3">
					...
					<div className="flex items-center flex-wrap gap-2 w-1/3 shrink-0 justify-end">
						<GlobalTaskStatus
							task={task}
							editable={true}
						...
						{task.organization && (
							<Link to={fullUrl} className="inline-block">
								<Button variant="primary" className="border-transparent h-7" size={"sm"}>
