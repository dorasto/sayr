...
function ReleaseDetailPageContent() {
	...
	const { organization, setOrganization, labels, categories, releases, setReleases } = useLayoutOrganization();
		...
		async (content: NodeJSON | undefined) => {
			...
			try {
				...
				const result = await runWithToast(
					...
					() => updateReleaseAction(organization.id, release.id, { description: processedContent }, wsClientId)
				);
			...
			const result = await runWithToast(
				"update-release-target-date",
				{
					loading: {
						...
						description: date ? "Setting target date." : "Clearing target date.",
					},
					success: {
				...
				() => updateReleaseAction(organization.id, release.id, { targetDate: date }, wsClientId)
			);
			...
			const result = await runWithToast(
				"update-release-released-date",
				{
					loading: {
						...
						description: date ? "Setting release date." : "Clearing release date.",
					},
					success: {
				...
				() => updateReleaseAction(organization.id, release.id, { releasedAt: date }, wsClientId)
			);
	...
	const handleHeaderUpdate = useCallback(
		async (data: { name: string; slug: string; icon: string; color: string }) => {
			if (!release) return;
	...
	return (
		<div className="relative flex flex-col h-full max-h-full">
			<ResizablePanelGroup direction="horizontal">
				...
				{useMobile ? (
					<Sheet defaultOpen={false} open={isChartsPanelOpen} onOpenChange={releaseChartsActions.close}>
						<SheetContent className="p-0" showClose={false}>
							<SheetHeader className="sr-only">
								<SheetTitle>Charts and Statistics</SheetTitle>
								<SheetDescription>View charts and statistics for this release</SheetDescription>
							</SheetHeader>
							<div className="h-full overflow-y-auto flex flex-col gap-3 p-3">
								<ReleaseInfo
									release={release}
									...
									onTargetDateUpdate={handleTargetDateUpdate}
								/>
								<ReleaseSidebar tasks={tasks} taskStats={taskStats} daysUntilTarget={daysUntilTarget} />
							</div>
						</SheetContent>
					</Sheet>
				) : (
