					...
					<div className="relative flex-1 flex flex-col min-h-0">
						...
						>
							...
							>
								...
								{/* Status — toolbar (draft mode, skips API calls, updates local state) */}
								<div className="flex items-center gap-2">
									<ReleaseFieldToolbar
										release={draftRelease}
										variant="toolbar"
										fields={["status"]}
										onChange={{ status: (s) => setStatus(s) }}
									/>
								</div>
							</div>
						</div>
						...
						<AdaptiveDialogFooter className="mt-auto bg-background flex flex-col! gap-2 shrink-0">
							<div className="flex items-center gap-2 w-full">
								...
								<p className="text-xs text-muted-foreground">
									{organization.slug}.{import.meta.env.VITE_ROOT_DOMAIN}/releases/{slug || "..."}
								</p>
