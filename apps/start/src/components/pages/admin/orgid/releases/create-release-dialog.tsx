...
export function CreateReleaseDialog({
	...
	const handleCreate = async () => {
		...
		if (description) {
			uploadedDescription = await processUploads(description, "public", organization.id, "create-release");
		}
		...
		if (data?.success && data.data) {
			...
			if (selectedTasks.length > 0) {
				postCreation.push(
					...selectedTasks.map((t) =>
						updateTaskAction(organization.id, t.id, { releaseId: data.data!.id }, sseClientId)
					)
				);
			}
			...
			if (status === "released" && releasedAt) {
				postCreation.push(updateReleaseAction(organization.id, data.data.id, { releasedAt }, sseClientId));
			}
	...
	const handleNameChange = (value: string) => {
		setName(value);
		if (!slug) {
			// Derived from the name field (not directly typed into), so generateSlug()'s
			// trailing-hyphen stripping doesn't fight live typing here — same reasoning as
			// create-organization-dialog.tsx's auto-generate-from-name effect.
				...
				>
					...
					<div className="relative flex-1 flex flex-col min-h-0">
						<div className={cn("flex flex-col gap-3 w-full p-3", !isMobile && "flex-1 min-h-0 pt-0!")}>
							<div className={cn("flex flex-col gap-3 w-full", !isMobile && "flex-1 min-h-0")}>
								{/* Description editor */}
							...
							<div className="flex items-center gap-2 w-full">
								...
								<div className="flex items-center gap-2 ml-auto">
									<Button
										...
										disabled={!name.trim() || !slug.trim() || isFetching || disabled}
