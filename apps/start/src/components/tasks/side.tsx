...
export default function ProjectSide() {
  ...
  // Helper to create release filter
          ...
          <div className="flex flex-col gap-0.5">
            {releases.length === 0 ? (
              ...
              releases.map((release) => {
                ...
                const isActive =
                  serializeFilters(filters) === serializeFilters(releaseFilter);
                const releaseTaskCount = tasks.filter(
                  (task) => task.releaseId === release.id,
                ).length;
