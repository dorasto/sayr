...
export function TaskVoting({
			...
			try {
				const data = await runWithToast(
					"update-task-vote",
					{
						loading: {
							...
							description: "Updating vote count... changes are already visible.",
						},
						success: {
							title: "Vote updated",
						...
						error: {
							...
							description: "Your vote is showing, but we couldn't save it to the server. Please try again.",
						},
					},
				...
				if (data?.success && data.data) {
					...
					const updatedTasks = tasks.map((task) =>
						task.id === data.data.taskId && task.organizationId === organizationId
							? {
									...task,
	...
	return (
		...
		>
			<IconChevronUp />
			<span className="text-xs font-normal">{task.voteCount?.toString() || "0"}</span>
		</Button>
	);
}
