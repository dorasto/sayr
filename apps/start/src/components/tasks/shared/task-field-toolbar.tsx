...
type FieldKey = "status" | "priority" | "labels" | "assignees" | "category" | "visibility" | "release";
...
export default function TaskFieldToolbar({
	...
	const debouncedUpdateLabels = useDebounceAsync(
		async (values: string[], _wsClientId: string) => {
			const data = await runWithToast(
				...
				() => updateLabelToTaskAction(task.organizationId, task.id, values, _wsClientId),
			);
			return data;
		},
		1500,
				...
				if (setTasks && setSelectedTask) {
					const updatedTasks = tasks.map((t) =>
						t.id === task.id
							? {
									...task,
									labels: availableLabels.filter((label) => values.includes(label.id)),
								}
							: t,
					);
					setTasks(updatedTasks);
			...
			{selectedAssignees.length > 1 ? (
				...
				<div className="flex items-center">
					<Avatar className="h-4 w-4 mr-2">
						<AvatarImage src={selectedAssignees[0]?.image || undefined} alt={selectedAssignees[0]?.name} />
						<AvatarFallback className="text-[8px]">
	...
	const creatorVisibilityTrigger = (
		<Button
			variant={"primary"}
			className={cn("w-fit text-xs h-7", task.visible === "private" && "border-primary/50")}
			size={"sm"}
			onClick={() => onChange?.visibility?.(task.visible === "private" ? "public" : "private")}
		>
		...
		<div className={style.container}>
			{fields.status && (
				<GlobalTaskStatus
					...
					{...(style.useCustomTrigger ? { customTrigger: creatorStatusTrigger } : {})}
				/>
			)}
			...
			{fields.priority && (
				<GlobalTaskPriority
					...
					{...(style.useCustomTrigger ? { customTrigger: creatorPriorityTrigger } : {})}
				/>
			)}
		...
		{fields.labels && (
			...
			{fields.assignees && (
				<GlobalTaskAssignees
					...
					{...(style.useCustomTrigger ? { customTrigger: creatorAssigneesTrigger } : {})}
				/>
			)}
			...
			{fields.category && (
				<GlobalTaskCategory
					...
					{...(style.useCustomTrigger ? { customTrigger: creatorCategoryTrigger } : {})}
				/>
			)}
			...
			{fields.visibility &&
				(style.useCustomTrigger ? (
					...
					<GlobalTaskVisibility {...sharedProps} onChange={onChange?.visibility} />
				))}
			...
			{fields.release && (
				<GlobalTaskRelease
					...
					{...(style.useCustomTrigger ? { customTrigger: creatorReleaseTrigger } : {})}
				/>
			)}
		</div>
	);
