...
export default function TaskFieldToolbar({
	...
	const priorityCfg = priorityConfig[resolvedPriority as keyof typeof priorityConfig];

	const selectedLabels = task.labels ?? [];
	const selectedAssignees = task.assignees ?? [];
	const selectedCategory = useMemo(() => categories.find((c) => c.id === task.category), [categories, task.category]);
	const selectedRelease = useMemo(() => releases.find((r) => r.id === task.releaseId), [releases, task.releaseId]);

	// ── Overflow menu logic (creator variant only) ─────────────────────
	// Track which overflow field picker is currently open
	const [overflowOpenField, setOverflowOpenField] = useState<FieldKey | null>(null);
	...
	const OVERFLOW_FIELD_META: Partial<Record<FieldKey, { icon: React.ReactNode; label: string }>> = {
		parent: {
	...
	const buildSharedProps = (cfg: FieldConfig) => {
		...
		const fieldShowLabel = isIconOnly ? false : (cfg.showLabel ?? style.showLabel);
		const fieldShowChevron = isIconOnly ? false : (cfg.showChevron ?? style.showChevron);
		const fieldClassName = cn(style.useCustomTrigger ? "" : style.className, cfg.className);
	...
	const FIELD_RENDERERS: Record<FieldKey, (cfg: FieldConfig) => React.ReactNode> = {
		status: (cfg) => (
			<GlobalTaskStatus
				{...buildSharedProps(cfg)}
				onChange={onChange?.status}
				{...(style.useCustomTrigger ? { customTrigger: creatorStatusTrigger(cfg, { statusCfg }) } : {})}
			/>
		),
		...
		priority: (cfg) => (
			<GlobalTaskPriority
				...
				{...(style.useCustomTrigger ? { customTrigger: creatorPriorityTrigger(cfg, { priorityCfg }) } : {})}
			/>
		),
		...
		assignees: (cfg) => (
			<GlobalTaskAssignees
				...
				{...(style.useCustomTrigger ? { customTrigger: creatorAssigneesTrigger(cfg, selectedAssignees) } : {})}
			/>
		),
		...
		category: (cfg) => (
			<GlobalTaskCategory
				...
				{...(style.useCustomTrigger ? { customTrigger: creatorCategoryTrigger(cfg, selectedCategory) } : {})}
			/>
		),
		...
		visibility: (cfg) =>
			style.useCustomTrigger ? (
				...
				creatorVisibilityTrigger(cfg, task.visible, {
					onToggle: () => onChange?.visibility?.(task.visible === "private" ? "public" : "private"),
				})
			) : (
				<GlobalTaskVisibility {...buildSharedProps(cfg)} onChange={onChange?.visibility} />
			),
			...
			return (
				<div className="relative group/release-link flex items-center gap-1">
					<GlobalTaskRelease
						...
						{...(style.useCustomTrigger ? { customTrigger: creatorReleaseTrigger(cfg, selectedRelease) } : {})}
						{...(isOverflowTriggered
							? {
									open: true,
					...
					{!style.useCustomTrigger && selectedRelease?.slug && (
						...
						>
							<Button variant="ghost" size="sm" className="h-[26px] aspect-square p-0 rounded-lg">
		...
		vote: (cfg) => {
			...
			return (
				<TaskVoting
					...
					compact={fieldCompact}
					iconOnly={isIconOnly}
					className={cn(style.useCustomTrigger ? "w-fit text-xs" : style.className, "", cfg.className)}
				/>
			);
		},
		...
		<div className={style.container}>
			...
			{isOverflowEnabled &&
				...
				(overflowOpenField ? (
					...
					(() => {
						const activeCfg = overflowFields.find((cfg) => cfg.key === overflowOpenField);
						if (!activeCfg) return null;
						const renderer = FIELD_RENDERERS[activeCfg.key];
						if (!renderer) return null;
						return (
							<React.Fragment key={`overflow-active-${activeCfg.key}`}>{renderer(activeCfg)}</React.Fragment>
						);
					})()
				) : (
