...
interface TaskPickerProps {
	...
	/** Optional — when provided, result rows show `{orgShortId}-{shortId}` instead of a bare shortId. */
	orgShortId?: string;
...
export default function TaskPicker({
	...
	const fetchResults = useCallback(
		async (searchQuery: string, signal?: AbortSignal) => {
			setIsLoading(true);
			try {
				const data = await searchOrgTasks(organizationId, searchQuery, 20, 0, signal);
				if (!signal?.aborted) {
					setResults(data);
				}
			} catch {
	...
	return (
		<ComboBox value={value || ""} onValueChange={handleChange} open={open} onOpenChange={onOpenChange}>
			{customTrigger ? (
			...
			<ComboBoxContent shouldFilter={false}>
				<ComboBoxSearch icon placeholder={searchPlaceholder} onValueChange={handleSearchChange} />
				<ComboBoxList>
					{isLoading ? (
						...
						<ComboBoxEmpty>{query.length > 0 ? "No tasks found" : "No recent tasks"}</ComboBoxEmpty>
					) : (
						<ComboBoxGroup>
							{filteredResults.map((t) => (
								<ComboBoxItem key={t.id} value={t.id}>
									<TaskPickerItem task={t} orgShortId={orgShortId} />
								</ComboBoxItem>
							))}
						</ComboBoxGroup>
					)}
...
export function TaskPickerItem({
	...
	/** When provided, the identifier shows as `{orgShortId}-{shortId}` instead of a bare shortId. */
	orgShortId?: string;
	className?: string;
}) {
