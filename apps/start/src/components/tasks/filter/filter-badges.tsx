...
export function FilterBadges(props: FilterBadgesProps) {
	...
	// On mobile, filters are shown inside the FilterMenu drawer, not as separate badges
	if (isMobile) {
		return null;
	}
						...
						if (labelObjs.length <= 2) {
							...
							displayNode = (
								...
								>
									...
									<span className="truncate max-w-56 pl-2">{labelObjs.length} labels</span>
								</span>
							);
						}
					}
				...
				return (
					<ButtonGroup key={condition.id} className="h-6 text-xs">
						...
						<DropdownMenu>
							...
							<DropdownMenuContent align="start" collisionPadding={16} className="w-40 max-w-[calc(100vw-2rem)]">
						...
						<Button
							variant="accent"
							size="sm"
							className={`${commonButtonClasses} hover:bg-destructive text-foreground ml-auto`}
