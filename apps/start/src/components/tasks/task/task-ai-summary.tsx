...
import { type AiPromptDebugInfo, fetchTaskSummaryStatus, streamSummarizeTask } from "@/lib/fetches/ai";
...
export function AiTaskSummary({ task, orgId, embedded }: AiTaskSummaryProps) {
	...
	const content = (
		<>
			...
			{renderedHtml && !loading && (
				<Tile className="border border-primary/30 rounded-lg py-1">
					<TileHeader>
						...
						<TileDescription>Review important information carefully.</TileDescription>
					</TileHeader>
				</Tile>
			)}
			...
			{generatedAt && !loading && (
				<p className="text-xs text-muted-foreground">Generated {formatDateTimeFromNow(generatedAt)}</p>
			)}
	...
	return (
		<div className="rounded-xl border border-dashed border-border bg-card p-3 flex flex-col gap-2">{content}</div>
	);
}
