...
/** Natural-language verb phrases used in toast copy, e.g. "Blocking SAY-19". */
const RELATION_TOAST_LABELS: Record<RecommendedRelation["type"], string> = {
	blocking: "Blocking",
	related: "Related to",
	duplicate: "Dupe of",
};

/** Group headings for relation suggestions, split into their own rows below. */
const RELATION_GROUP_LABELS: Record<RecommendedRelation["type"], string> = {
	blocking: "Blocking",
	related: "Related",
...
interface SuggestionChipProps {
	...
	/**
	 * Optional short, secondary bit of context shown before `label` in a
	 * visually distinct style (monospace, muted) — e.g. a task key like
	 * "SAY-19" ahead of the task's title. Purely cosmetic separation, not a
	 * semantic difference from `label`.
	 */
	meta?: string;
	label: string;
	onApply: () => void;
		...
		>
			<button type="button" onClick={onApply} className="flex items-center gap-1.5 min-w-0 cursor-pointer">
				...
				<span className="truncate">
					{meta && <span className="font-mono text-[10px] text-muted-foreground mr-1.5">{meta}</span>}
					{label}
				</span>
			</button>
			<button
		...
		execute({
			...
			toastMessages: {
				...
				success: {
					...
					description: `${RELATION_TOAST_LABELS[relation.type]} ${relation.shortId ? formatTaskKey(org?.shortId ?? "", relation.shortId) : relation.title}`,
				},
			...
			{hasContent ? (
				<div className="flex flex-col gap-2">
					{suggestedLabels.length > 0 && (
						<div className="flex flex-col gap-1.5">
							<div className="flex flex-wrap items-center gap-2">
								<span className="text-xs text-foreground font-semibold">Labels:</span>
								{suggestedLabels.map((label) => (
									<SuggestionChip
										...
										label={label.name}
										onApply={() => r.applyLabel(label.id)}
										onDismiss={() => r.dismissLabel(label.id)}
									/>
								))}
							</div>
						</div>
					...
					{suggestedAssignees.length > 0 && (
						<div className="flex flex-col gap-1.5">
							<div className="flex flex-wrap items-center gap-2">
								<span className="text-xs text-foreground font-semibold">Assignees:</span>
					...
					{suggestedCategory && (
						<div className="flex flex-col gap-1.5">
							<div className="flex flex-wrap items-center gap-2">
								<span className="text-xs text-foreground font-semibold">Category:</span>
								<SuggestionChip
					...
					{suggestedRelease && (
						<div className="flex flex-col gap-1.5">
							<div className="flex flex-wrap items-center gap-2">
								<span className="text-xs text-foreground font-semibold">Release:</span>
								<SuggestionChip
					...
					{/* One row per relation type (blocking/duplicate/related) rather than a
					    single mixed "Relations:" row — groups read faster when a task has
					    more than one kind of relation suggested at once. */}
					{(["blocking", "duplicate", "related"] as const).map((type) => {
						const relationsOfType = suggestedRelations.filter((relation) => relation.type === type);
						if (relationsOfType.length === 0) return null;
						return (
							<div key={type} className="flex flex-col gap-1.5">
								<div className="flex flex-wrap items-center gap-2">
									<span className="text-xs text-foreground font-semibold">{RELATION_GROUP_LABELS[type]}:</span>
									{relationsOfType.map((relation) => (
										<SuggestionChip
											key={relation.taskId}
											icon={RELATION_ICONS[relation.type]}
											meta={formatTaskKey(r.orgShortId, relation.shortId)}
...
/**
 * "Recommendations" — a quiet `ContextSection` row (same visual language as
 * `TaskContextBanner`'s "Related to"/"Duplicate of"/etc. rows), used both on
 * its own and composed inside `AiInsights` — there's no separate card
 * treatment to opt in/out of anymore.
 *
 * Non-admins get a fully silent feature — it only ever renders once there's
 * something to show. Admins always keep it reachable, so the
 * Regenerate/View-prompt controls stay available.
 */
export function AiRecommendations({ isProjectAdmin, ...hookProps }: AiRecommendationsProps) {
