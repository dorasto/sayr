...
interface TaskContextBannerProps {
	...
	/** Optional — falls back to `task.organization` when absent, same as TaskDetailCompact. */
...
function RelationTaskRow({
	...
	orgId: string;
	/** Undefined when no org context is available — falls back to a bare shortId. */
	orgShortId?: string;
	tasks: schema.TaskWithLabels[];
	setTasks: (tasks: schema.TaskWithLabels[]) => void;
	setSelectedTask: (task: schema.TaskWithLabels | null) => void;
}) {
	const fullTask = useMemo(() => tasks.find((t) => t.id === taskData.id), [tasks, taskData.id]);
		...
		<div className="flex items-center gap-1.5 py-1 group text-sm  p-1 rounded-lg bg-transparent hover:bg-muted transition-all w-full">
			{fullTask ? (
				<GlobalTaskStatus
					...
					customTrigger={
						<button type="button" className="size-4 grid place-items-center shrink-0 cursor-pointer">
...
export function TaskContextBanner({ task, tasks, setTasks, setSelectedTask, organization }: TaskContextBannerProps) {
	const orgShortId = (organization ?? task.organization)?.shortId;
	...
	const blockedByRelations = useMemo(
		() => relations.filter((r) => r.type === "blocking" && r.direction === "target"),
		[relations]
	);

	const blockingRelations = useMemo(
		() => relations.filter((r) => r.type === "blocking" && r.direction === "source"),
		[relations]
	);

	const relatedRelations = useMemo(() => relations.filter((r) => r.type === "related"), [relations]);

	const duplicateRelations = useMemo(() => relations.filter((r) => r.type === "duplicate"), [relations]);
	...
	const originIntegrations = getMatchedIntegrations(integrationActivities ?? []);
	...
	return (
		<div className="flex flex-col gap-3">
			...
			{/* Blocking section */}
			{hasBlocking && (
				<ContextSection label="Blocking" icon={<IconArrowUpRight className="h-3 w-3 text-muted-foreground" />}>
			...
			{/* Related section */}
			{hasRelated && (
				<ContextSection label="Related to" icon={<IconLink className="h-3 w-3 text-muted-foreground" />}>
			...
			{hasSubtasks && subtaskProgress && (
				<ContextSection
					label="Sub tasks"
					trailing={<SubtaskProgressBadge completed={subtaskProgress.completed} total={subtaskProgress.total} />}
				>
