...
/**
 * "AI Insights" — hosts the task's AI Summary and Recommendations sections.
 * Computes the org-wide gates (AI enabled, plan, disabled, rate-limited)
 * once, shared by both sections, so the org only ever sees ONE upsell/
 * rate-limited notice rather than one per feature.
 *
 * Deliberately not a card: both sections render as their own quiet
 * `ContextSection` row (same visual language as `TaskContextBanner`'s
 * "Related to"/"Duplicate of"/etc. rows) so AI content reads as part of the
 * page rather than a separate, visually louder feature competing with the
 * task's own title/description for attention.
 */
export function AiInsights({
	...
	if (!taskSummaryEnabled) {
		// Nothing to pair Recommendations with — render it alone.
		return <AiRecommendations {...recommendationsProps} />;
	}

	// Same quiet stack as TaskContextBanner's "Related to"/"Duplicate of"/etc.
	// rows — AI Summary and Recommendations are just two more ContextSection
	// rows on the page, not a separate visually-loud feature.
	return (
		<div className="flex flex-col gap-3">
			<AiTaskSummary task={task} orgId={orgId} embedded />
			<AiRecommendations {...recommendationsProps} />
		</div>
	);
}
