...
export default function CreateIssueDialog({
	...
	}, [open, organization?.id, organization?.shortId, setMentionContext]);
	...
	const [templateData, setTemplateData] = useState<undefined | NodeJSON>(undefined);
	...
	const [parentTask, setParentTask] = useState<OrgTaskSearchResult | null>(null);
	...
	const handleUpdate = async () => {
		...
		if (data?.success && data.data) {
			...
			headlessToast.success({
				...
				description: extractTaskText(createdTask.description) || "The task has been successfully created.",
				action: {
					label: "View task",
					onClick: () => {
						navigate({
			...
			<AdaptiveDialog open={open} onOpenChange={setOpen}>
				...
				>
					<motion.div
						...
						initial={false}
						animate={{
							width: !isMobile ? (expand ? DIALOG_SIZES.expanded.width : DIALOG_SIZES.collapsed.width) : "100%",
						}}
						transition={DIALOG_SIZES.transition}
						style={{
							height: !isMobile ? (expand ? DIALOG_SIZES.expanded.height : "auto") : "auto",
							minHeight: !isMobile
								? expand
					...
					>
								...
								<div className="flex items-center gap-1 w-full">
									{organizations && organizations.length > 0 && (
										<ComboBox
											...
											onValueChange={(val) => {
												if (val && val !== organization.id && onOrganizationChange) {
										...
										>
											<ComboBoxTrigger className="w-fit text-xs h-7 border border-transparent hover:border-border bg-accent text-accent-foreground hover:bg-secondary rounded-lg px-2 mb-0 flex items-center gap-2">
												...
												<ComboBoxValue placeholder="Select org...">{organization.name}</ComboBoxValue>
											</ComboBoxTrigger>
											<ComboBoxContent>
										...
										>
											...
											>
												...
												<ComboBoxValue placeholder="Choose a template...">
													{selectedTemplateId === "__none__"
														? "Template"
														: issueTemplates.find((t) => t.id === selectedTemplateId)?.name ||
															"Choose a template..."}
												</ComboBoxValue>
											</ComboBoxTrigger>
											<ComboBoxContent>
												...
												<ComboBoxList>
													...
													<ComboBoxGroup>
														<ComboBoxItem value="__none__">No template</ComboBoxItem>
									...
									<div className="flex items-center gap-1 ml-auto">
										<Button
											...
											onClick={() => setVisible(visible === "public" ? "private" : "public")}
										>
											{visible === "private" ? (
												<>
						...
						<div className="relative flex-1 flex flex-col min-h-0">
							...
							<div className={cn("flex flex-col gap-3 w-full p-3", !isMobile && "flex-1 min-h-0 pt-0!")}>
								<div className={cn("flex flex-col gap-1 w-full", !isMobile && "flex-1 min-h-0")}>
									<Input
										variant={"strong"}
									...
									/>
									<div className={cn("w-full transition-all", !isMobile && "flex-1 min-h-24 overflow-y-auto")}>
										<Editor
