...
export function CommentReplyInput({ parentComment, categories, tasks, onReplyPosted }: CommentReplyInputProps) {
	...
	const handleSubmit = async () => {
		...
		if (data?.success) {
			...
			queryClient.invalidateQueries({
				queryKey: ["timeline", "comments", parentComment.taskId, parentComment.organizationId],
			});
			onReplyPosted?.();
		}
	};
