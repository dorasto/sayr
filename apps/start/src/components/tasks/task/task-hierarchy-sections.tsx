...
interface HierarchySectionProps {
	...
	/** Optional — when provided, picker/item identifiers show `{orgShortId}-{shortId}` instead of a bare shortId. */
	orgShortId?: string;
}
...
export function TaskParentSection({
	...
	const handleSetParent = async (parentTask: OrgTaskSearchResult) => {
		...
		const data = await runWithToast(
			...
			() => setTaskParentAction(task.organizationId, task.id, parentTask.id, sseClientId)
		);
				...
				<TileAction>
					{task.parent ? (
						<div className="flex items-center gap-1 group/parent w-full">
							...
							>
								<TaskPickerItem task={task.parent} orgShortId={orgShortId} />
							</Link>
							<Button
								variant="ghost"
								size="icon"
...
export function TaskSubtasksSection({
	...
	const handleAddSubtask = async (childTask: OrgTaskSearchResult) => {
		...
		const data = await runWithToast(
			...
			() => setTaskParentAction(task.organizationId, childTask.id, task.id, sseClientId)
		);
				...
				<TileAction>
					<div className="flex flex-col gap-0.5 w-full">
						{subtasks.map((subtask) => (
							<div key={subtask.id} className="flex items-center gap-1 group/subtask w-full">
								<Link
									to="/$orgId/tasks/$taskShortId"
									params={{
										orgId: task.organizationId,
									...
									className={SIDEBAR_ITEM_CLASS}
								>
									<TaskPickerItem task={subtask} orgShortId={orgShortId} />
								</Link>
								<Button
									variant="ghost"
									size="icon"
...
function getRelationIcon(relation: schema.TaskRelationWithTarget): React.ReactNode {
...
function groupRelations(relations: schema.TaskRelationWithTarget[]): Map<string, schema.TaskRelationWithTarget[]> {
...
export function TaskRelationsSection({
	...
	const handleAddRelation = async (targetTask: OrgTaskSearchResult) => {
		...
		const data = await runWithToast(
			...
			() => createTaskRelationAction(task.organizationId, task.id, targetTask.id, type, sseClientId)
		);
	...
	const handleRemoveRelation = async (relation: schema.TaskRelationWithTarget) => {
		...
		const data = await runWithToast(
			...
			() => removeTaskRelationAction(task.organizationId, relation.id, sourceTaskId, targetTaskId, sseClientId)
		);
	...
	return (
		<>
			...
			{Array.from(groupedRelations.entries()).map(([groupLabel, groupRelations]) => (
						...
						<TileAction>
							<div className="flex flex-col gap-0.5 w-full">
								{groupRelations.map((relation) => (
									<div key={relation.id} className="flex items-center gap-1 group/relation w-full">
										<Link
											to="/$orgId/tasks/$taskShortId"
											params={{
												orgId: task.organizationId,
										...
										>
											...
											<TaskPickerItem task={relation.task} orgShortId={orgShortId} className="flex-1" />
										</Link>
										<Button
											variant="ghost"
											size="icon"
