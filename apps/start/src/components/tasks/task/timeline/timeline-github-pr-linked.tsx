...
function parsePRData(item: TimelineItemProps["item"]): GithubPRLinkedData | null {
	...
	return (
		...
		>
			...
			<a href={data.pullRequest.url} target="_blank" rel="noopener noreferrer">
				<InlineLabel text={`#${data.pullRequest.number}`} icon={<IconBrandGithub size={12} />} />
			</a>{" "}
			from <strong>{data.pullRequest.headBranch}</strong> → <strong>{data.pullRequest.baseBranch}</strong>
		</TimelineItemWrapper>
	);
}
