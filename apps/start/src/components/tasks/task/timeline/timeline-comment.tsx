...
import { IconArrowRight, IconDots, IconHistory, IconMessageDots, IconPencil } from "@tabler/icons-react";
			...
			<DialogContent className="sm:max-w-2xl max-h-[80vh] flex flex-col overflow-hidden p-0 gap-0">
				<DialogHeader className="p-3 border-b">
					...
					<DialogDescription asChild>
						<Label variant={"description"}>Previous versions of this comment.</Label>
					</DialogDescription>
				</DialogHeader>
				...
				<div className="flex-1 overflow-y-auto p-3">
					...
					{!loading && !error && history.length === 0 ? (
						...
						<p className="text-sm text-muted-foreground">{history.length} edits</p>
					)}
								...
								>
									<CollapsibleTrigger asChild>
										<div className="flex items-center gap-2 w-full data-[state=open]:[&_svg]:rotate-90">
											<InlineLabel
												text={actor?.name || ""}
												textNode={<InlineLabel text={actor?.name || ""} image={actor?.image} />}
												icon={<IconArrowRight className="size-3 transition-all text-foreground" />}
											/>
...
export function TimelineComment({
	...
	async function handleSave() {
		...
		const data = await runWithToast(
			...
			() => UpdateTaskCommentAction(item.organizationId, task.id, item.id, processed, item.visibility, wsClientId)
		);
	...
	const handleToggleReaction = useCallback(
		async (commentId: string, emoji: ReactionEmoji) => {
			...
			const previousData = queryClient.getQueryData<InfiniteData<CommentsPageData>>(queryKey);
				...
				return {
					...
					pages: old.pages.map((page) => ({
						...
						data: page.data.map((comment) => {
							...
							return {
								...comment,
								reactions: total > 0 ? { commentId, total, reactions: newReactions } : undefined,
							};
						}),
					})),
				};
