...
import { type ReactionEmoji, ReactionPicker } from "./reactions";
				...
				<div className="flex-1 overflow-y-auto p-3 space-y-4">
					...
					{!loading && !error && diffs.length > 0 && (
						<>
							...
							{diffs.map((diff) => {
								const actor = diff.editedBy ? availableUsers?.find((user) => user.id === diff.editedBy) : null;

								const editTime = diff.editedAt ? formatDateTimeFromNow(diff.editedAt) : "Unknown time";

								return (
									<div key={diff.id} className="border rounded-lg overflow-hidden">
										<div className="flex items-center justify-between gap-2 p-2 bg-accent/50 border-b">
											<div className="flex items-center gap-2">
												{actor && <InlineLabel text={getDisplayName(actor)} image={actor.image} />}
												<Tooltip>
													<TooltipTrigger>
														<Label variant={"description"}>{editTime}</Label>
													</TooltipTrigger>
													<TooltipContent>{formatDateTime(diff.editedAt as Date)}</TooltipContent>
												</Tooltip>
											</div>
										</div>
					...
					{!loading && !error && history.length > 0 && diffs.length === 0 && (
						<p className="text-sm text-muted-foreground">
							{history.length} {history.length === 1 ? "edit" : "edits"} recorded, but no previous version
...
export function TimelineComment({
	...
	const [editedContent, setEditedContent] = useState<NodeJSON | undefined>(item.content as NodeJSON | undefined);
	...
	const canSave = newCommentText.trim().length > 0 && oldCommentText !== newCommentText;

	const task = tasks?.find((t) => t.id === item.taskId);
	const { value: account } = useStateManagement<schema.userType>("account", null);
	...
	const canManageComment = useMemo(() => {
		...
		const isMember = hasMembers ? organization.members.some((m) => m.user?.id === account.id) : false;
		return !!isMember;
	...
	async function handleSave() {
		...
		const data = await runWithToast(
			...
			() => UpdateTaskCommentAction(item.organizationId, task.id, item.id, processed, item.visibility, sseClientId)
		);
	...
	async function handleDelete() {
		...
		const data = await runWithToast(
			...
			() => DeleteTaskCommentAction(item.organizationId, task.id, item.id, sseClientId)
		);
	...
	async function handleToggleVisibility() {
		...
		const data = await runWithToast(
			...
			() => UpdateCommentVisibilityAction(item.organizationId, task.id, item.id, newVisibility, sseClientId)
		);
	...
	const handleToggleReaction = useCallback(
		async (commentId: string, emoji: ReactionEmoji) => {
			...
			if (isReply && item.parentId) {
				...
				const replyQueryKey = ["comment-replies", item.parentId, item.organizationId];
				previousData = queryClient.getQueryData<schema.taskTimelineWithActor[]>(replyQueryKey);

				queryClient.setQueryData<schema.taskTimelineWithActor[]>(replyQueryKey, (old) => {
					if (!old) return old;
					return old.map(applyReactionToggle);
				});

				try {
					await CreateTaskReactionAction(item.organizationId, item.taskId, commentId, emoji, sseClientId);
				} catch {
				...
				const queryKey = ["timeline", "comments", item.taskId, item.organizationId];
				previousData = queryClient.getQueryData<InfiniteData<CommentsPageData>>(queryKey);

				queryClient.setQueryData<InfiniteData<CommentsPageData>>(queryKey, (old) => {
					if (!old) return old;
		...
		<>
			...
			<AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							This action cannot be undone. This will permanently delete the comment and all of its edit history.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
			...
			<AlertDialog open={visibilityDialogOpen} onOpenChange={setVisibilityDialogOpen}>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>
							<Label variant={"heading"}>Change to {item.visibility === "public" ? "internal" : "public"}</Label>
						</AlertDialogTitle>
						<AlertDialogDescription>
