...
function parseBranchData(item: TimelineItemProps["item"]): GithubBranchLinkedData | null {
...
export function TimelineGithubBranchLinked({
	...
	const authorName = hasActor ? getDisplayName(item.actor!) : data.author || "Someone";
	...
	return (
		...
		>
			<InlineLabel text={authorName} image={authorImage} className="text-muted-foreground hover:text-foreground" />{" "}
			{isDeleted ? "deleted branch" : "linked branch"}{" "}
			<a href={data.branch.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1">
				<InlineLabel
					text=""
					...
					textNode={
						<span className="flex items-center min-w-0 overflow-hidden!">
							<span className="truncate">
								{data.repository.owner}/{data.repository.name}:{data.branch.name}
							</span>
						</span>
					}
				/>
