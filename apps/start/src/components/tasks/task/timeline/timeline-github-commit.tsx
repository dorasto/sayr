...
function parseCommitData(item: TimelineItemProps["item"]): CommitRefData | null {
...
export function TimelineGithubCommit({ item, showSeparator = true }: TimelineItemProps & { showSeparator?: boolean }) {
	const commitData = parseCommitData(item);
	if (!commitData) return null;

	const hasActor = !!item.actor;
	const authorName = hasActor ? getDisplayName(item.actor!) : commitData.author || "Someone";
				...
				<InlineLabel
					...
					textNode={
						<span className="flex items-center min-w-0 overflow-hidden!">
							<span className="truncate">{commitData.message.split("\n")[0]}</span>
						</span>
					}
				/>
			</a>
