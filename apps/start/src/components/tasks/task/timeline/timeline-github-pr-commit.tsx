...
function parsePRCommitData(item: TimelineItemProps["item"]): GithubPRCommitData | null {
...
export function TimelineGithubPRCommit({
	...
	return (
		...
		>
			New commit{" "}
			<a href={`${data.pullRequest.url}/commits/${data.commit.sha}`} target="_blank" rel="noopener noreferrer">
				<InlineLabel text={shortSha} icon={<IconBrandGithub size={12} />} />
			</a>{" "}
