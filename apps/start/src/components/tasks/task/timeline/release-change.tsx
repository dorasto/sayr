				...
				<>
					...
					{toRelease ? (
						<InlineLabel
							...
							icon={
								toRelease.icon ? (
									<RenderIcon iconName={toRelease.icon} size={12} color={toRelease.color || undefined} raw />
								) : (
									<div
						...
						<InlineLabel text={`Unknown (${toId})`} icon={<IconRocket size={12} />} />
					)}
				</>
			);
		}
			...
			return (
				<>
					...
					{fromRelease ? (
						...
						<InlineLabel text={`Unknown (${fromId})`} icon={<IconRocket size={12} />} />
					)}
				</>
			);
		}
		...
		return (
			<>
				...
				{fromRelease ? (
					...
					<>
						from <InlineLabel text={`Unknown (${fromId})`} icon={<IconRocket size={12} />} />{" "}
					</>
				) : null}
				{toRelease ? (
					<>
					...
					<>
						to <InlineLabel text={`Unknown (${toId})`} icon={<IconRocket size={12} />} />
					</>
				) : null}
			</>
		);
