...
import { extractHslValues, formatCount, formatDateCompact, formatTaskKey } from "@repo/util";
import {
	IconAppWindow,
	IconArrowRight,
	IconCategory,
...
interface UnifiedTaskItemProps {
	...
	onAddRelation?: (sourceTaskId: string, targetTaskId: string, type: "related" | "blocking" | "duplicate") => void;
	...
	/**
	 * Single-org contexts pass this; cross-org ("personal") contexts leave it
	 * undefined and fall back to the minimal `task.organization` instead.
	 */
...
export function UnifiedTaskItem({
	...
	// Cross-org ("personal": home, my-tasks) rows carry their own org on
	// task.organization instead of receiving the single-org `organization` prop.
	const orgShortId = organization?.shortId ?? task.organization?.shortId;
	const taskKey = orgShortId ? formatTaskKey(orgShortId, task.shortId) : task.shortId;
	...
	const subtaskProgress = useMemo(() => {
		...
		const completed = childTasks.filter((t) => t.status === "done" || t.status === "canceled").length;
				...
				<div className="flex shrink-0 items-center gap-2">
					<div className="relative flex flex-wrap grow shrink-0 items-center gap-2 whitespace-nowrap">
						...
						<GlobalTaskAssignees
							...
							side="left"
							customTrigger={<AssigneeAvatarTrigger assignees={task.assignees || []} />}
						/>
					</div>
				</div>
			</div>
	...
	const renderListContent = () => (
		...
		>
			<div
				className={cn(
					"px-2 pr-4 group/list-block relative flex gap-3 bg-transparent hover:bg-accent py-1 text-xs! transition-colors flex-row items-center data-[state=open]:bg-accent rounded-lg",
			...
			>
								...
								<div className="relative shrink-0 flex">
									<TriStateCheckbox
										...
										className={cn(
											...
											isSelected && "opacity-100 bg-primary/20 border-primary pointer-events-auto",
				...
				<div className="flex shrink-0 items-center gap-2">
					<div className="relative flex flex-wrap grow shrink-0 items-center gap-2 whitespace-nowrap">
						{subtaskProgress && (
							<SubtaskProgressBadge completed={subtaskProgress.completed} total={subtaskProgress.total} />
						)}
						...
						/>
						{/* Release */}
						<ReleaseBadgeButton releaseId={task.releaseId} releases={releases} onClick={handleReleaseClick} />
						{/* Labels */}
						<TaskLabelsInline
							labels={task.labels || []}
							maxVisible={2}
						...
						<div className="flex items-center text-muted-foreground">
							...
							<span className="text-xs truncate">{formatCount(task.voteCount)}</span>
						</div>
					</div>
				</div>
			</div>
		...
		>
			...
			<div className="flex items-center justify-between mt-auto pt-2">
				<div className="flex items-center gap-1">
					...
					<CategoryBadgeButton categoryId={task.category} categories={categories} onClick={handleCategoryClick} />
					<ReleaseBadgeButton releaseId={task.releaseId} releases={releases} onClick={handleReleaseClick} />
				</div>
				<GlobalTaskAssignees
					task={task}
	...
	const assigneeOptions = useMemo(() => getAssigneeOptionsFromUsers(availableUsers), [availableUsers]);
	const labelOptions = useMemo(() => getLabelOptions(availableLabels), [availableLabels]);
	...
	const parentOptions = useMemo(() => getParentOptions(task, tasks, orgShortId), [task, tasks, orgShortId]);
	const relationTypeOptions = useMemo(() => getRelationTypeOptions(), []);
	const relationTargetOptions = useMemo(
		() => getRelationTargetOptions(task, tasks, orgShortId),
		[task, tasks, orgShortId]
	);
		...
		<ContextMenuContent className="w-56">
			...
			<ContextMenuSub>
				...
				<ContextMenuSubContent className="w-44">
					<ContextMenuGroup>
						<Link to="/$orgId/tasks/$taskShortId" params={{ orgId: task.organizationId, taskShortId: taskId }}>
						...
						{onOpenInDialog && (
							<ContextMenuItem className="gap-3 w-full" onClick={() => onOpenInDialog(task)}>
						...
						<ContextMenuRadioGroup
							...
							onValueChange={(value) => {
								onTaskUpdate?.(task.id, {
									priority: (value === "none" ? null : value) as typeof task.priority,
								});
							}}
						>
				...
				<ContextMenuSub>
					...
					<ContextMenuSubContent className="w-52 max-h-60 overflow-y-auto pt-0">
						...
						{assigneeOptions.length > 0 ? (
							assigneeOptions
								.filter((opt) => opt.label.toLowerCase().includes(assigneeSearch.toLowerCase()))
								.sort((a, b) => {
				...
				<ContextMenuSub>
					...
					<ContextMenuSubContent className="w-52 max-h-60 overflow-y-auto pt-0">
						...
						{labelOptions.length > 0 ? (
							labelOptions
								.filter((opt) => opt.label.toLowerCase().includes(labelSearch.toLowerCase()))
								.sort((a, b) => {
										...
										<ContextMenuCheckboxItem
											...
											onSelect={(e) => {
												...
												const newIds = isApplied
													? currentIds.filter((id) => id !== opt.id)
													: [...currentIds, opt.id];
												const newLabels = availableLabels.filter((l) => newIds.includes(l.id));
			...
			{(fieldPermissions?.release ?? true) && (
				<ContextMenuSub>
					...
					<ContextMenuSubContent className="w-72 max-h-60 overflow-y-auto pt-0">
						...
						>
							{releaseOptions
								.filter((opt) => opt.label.toLowerCase().includes(releaseSearch.toLowerCase()))
								.sort((a, b) => {
													...
													<div className="flex items-center gap-1 ml-auto">
														...
														<Badge
															className={cn(
																...
																releaseStatusConfig[meta.status as keyof typeof releaseStatusConfig]
																	?.badgeClassName
															)}
														>
															{releaseStatusConfig[
