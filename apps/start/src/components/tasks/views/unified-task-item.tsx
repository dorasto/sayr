...
import { IconAppWindow, IconChevronUp, IconCircleFilled, IconLink, IconUserOff } from "@tabler/icons-react";
...
export function UnifiedTaskItem({
	...
	const renderCompactListContent = () => (
		<Link
			className={cn(
				"block cursor-pointer w-full text-left bg-transparent border-none p-0 outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded-lg",
		...
		>
			<div
				className={cn(
					"p-1 group/list-block h-10 max-h-10 relative flex gap-2 bg-transparent hover:bg-secondary text-sm transition-colors flex-row items-center rounded data-[state=open]:bg-accent",
			...
			>
				...
				<div className="flex gap-2 w-full truncate">
					<div className="flex grow items-center gap-2 truncate">
						...
						{/* Title */}
						<p className="truncate cursor-pointer text-sm text-foreground w-fit">{task.title}</p>
					</div>
				</div>
				...
				<div className="flex gap-2 w-full truncate">
					<div className="flex grow items-center gap-1 truncate">
						...
						{personal && task.organization && (
							<a href={`/${task.organization.id}/tasks`} onClick={(e) => e.stopPropagation()}>
								<Badge
									variant={"secondary"}
				...
				<div className="flex shrink-0 items-center gap-2">
					<div className="relative flex flex-wrap grow shrink-0 items-center gap-2 whitespace-nowrap">
						<div className="flex items-center text-muted-foreground">
							...
							<span className="text-xs truncate">{task.voteCount.toString()}</span>
						</div>
						{/* Category */}
						{task.category &&
							(() => {
	...
	const renderKanbanContent = () => (
		...
		>
			...
			<div className="font-medium text-xs line-clamp-2 leading-tight">{task.title}</div>

			{task.labels && task.labels.length > 0 && (
				<div className="flex flex-wrap gap-1 mt-1">
					{task.labels.slice(0, 3).map((label) => (
						<RenderLabel key={label.id} label={label} className="max-w-[100px]" data-no-propagate />
					))}
					{task.labels.length > 3 && (
