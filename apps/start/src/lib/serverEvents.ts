...
const useServerEvents = (initialOrgId?: string, initialChannel?: string) => {
	...
	const { setValue: setStatus } = useStateManagement<string>("sse-status", "Disconnected");

	const { setValue: setClientId } = useStateManagement<string>("sse-clientId", "");

	const disconnectTimeRef = useRef<number | null>(null);
	const reconnectTimerRef = useRef<NodeJS.Timeout | null>(null);
	const toastTimerRef = useRef<NodeJS.Timeout | null>(null);

	const connect = (orgId?: string, channel?: string, orgIds?: string[]) => {
		setParams({ orgId, channel, orgIds });
	};
	...
	useEffect(() => {
		const { orgId, channel, orgIds } = params;
		...
		const connectInternal = () => {
			...
			const hasOrgIds = orgIds && orgIds.length > 0;
			const url = hasOrgIds
				? `${API_URL}/events?orgIds=${encodeURIComponent(orgIds.join(","))}&channel=${encodeURIComponent(
						channel || "public"
					)}`
				: orgId || channel
					? `${API_URL}/events?orgId=${encodeURIComponent(
							orgId || ""
							...
							if (data.data.authenticated) {
								...
								if (disconnectTimeRef.current !== null) {
									...
									reconnectTimerRef.current = setTimeout(() => {
										sendWindowMessage(window, { type: "SSE_RECONNECTED" }, "*");
