...
Route.post(
	...
	async (c) => {
			...
			return c.json(
				{
					...
					message: "You are not authorized to create tasks in this organization.",
				},
				401
			);
		}
		...
		if (!task) {
			return c.json(
				{ success: false, error: "Failed to create task", message: "An error occurred while creating the task" },
				500
			);
		}
		...
		if (integration) {
			await traceAsync(
				...
				() =>
					addLogEventTask(task.id, orgId, "integration", null, { ...integration, createdBy }, userId || undefined),
				{
...
Route.post(
	...
	async (c) => {
		...
		// uses (task.ts `/create-comment`, which gates on "members"). This previously
		// required "tasks.create", so a plain member who could comment in the app was
		// rejected here.
		const isAuthorized = await traceOrgPermissionCheck(apiKeyResult.key.referenceId || "", orgId, "members");
