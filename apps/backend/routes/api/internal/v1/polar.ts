...
/**
 * POST /polar/subscription/revoke
 * Immediately revokes (cancels) the org's Polar subscription and
 * performs the same local cleanup as the webhook cancellation handler:
 * downgrades to free, reassigns seats by priority.
 * This is a testing-only endpoint — remove before production launch.
 * Body: { orgId: string }
 */
app.post("/subscription/revoke", async (c) => {
	const session = c.get("session");
	...
	try {
		...
		// 2. Downgrade org locally (same as handleSubscriptionCanceled)
		...
		// Sort: billing customer > administrators > billing perm > oldest
		const billingCustomerEmail = org.polarCustomerId
		...
		const sorted = [...orgMembers].sort((a, b) => {
			const aIsBillingCustomer = billingCustomerEmail && a.user.email?.toLowerCase() === billingCustomerEmail ? 1 : 0;
			const bIsBillingCustomer = billingCustomerEmail && b.user.email?.toLowerCase() === billingCustomerEmail ? 1 : 0;
		...
		console.log(
			`❌ Subscription revoked for org ${orgId}. Auto-assigned ${seatedIds.size} seats to priority members out of ${sorted.length} total.`,
		);

		return c.json({ success: true });
	} catch (err) {
		console.error("Failed to revoke subscription on Polar", err);
