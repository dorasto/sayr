...
// Accepts `undefined` so callers with no SSE client to exclude (e.g. `/v1/me`
// API writes) can pass `sseClientId` straight through — `undefined` naturally
// finds nothing, so every connected client (including the caller's own future
// SSE connections) still receives the broadcast.
export function findClientBysseId(id: string | undefined): SSEClient | undefined {
...
function getDeviceType(userAgent: string | undefined): string {
	...
	const isMobile = ua.includes("iphone") || ua.includes("ipad") || ua.includes("android") || ua.includes("mobile");
	...
	const stream = new ReadableStream({
		start(controller) {
			...
			const client: SSEClient = {
				...
				send,
				close,
				clientId: session?.user.id || userId || "",
				authenticated,
