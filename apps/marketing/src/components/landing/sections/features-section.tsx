			...
			{items.map((item) => (
				<div
					...
					className={`flex items-center justify-between py-1.5 px-3 rounded-md text-xs ${
						item.vis === "public" ? "bg-muted/50" : "bg-muted/30 border border-dashed border-border"
					}`}
				>
					<span className={item.vis === "public" ? "text-foreground" : "text-muted-foreground"}>
						{item.name}
					</span>
			...
			<div className="max-w-(--breakpoint-lg) mx-auto">
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
					...
					<FeatureCard
						...
						description="WebSocket-powered live collaboration. Every change is instant — no refresh needed."
					/>
