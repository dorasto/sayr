import { ArrowUpRight, Check, Cloud, Server, Sparkles } from "lucide-react";
...
function TierCard({ tier }: { tier: (typeof pricingData.tiers)[number] }) {
	...
	return (
		...
		>
			{isHighlighted && (
				<Badge className="absolute -top-3 right-6 text-xs shadow-lg shadow-primary/20">Recommended</Badge>
			)}
...
function SelfHostedCard({ tier }: { tier: (typeof pricingData.tiers)[number] }) {
	return (
...
function getHighlightsForTier(tierId: string): string[] {
	switch (tierId) {
		...
		case "enterprise":
			return ["Everything in Pro", "Self-hosted deployment", "Priority support", "License key activation"];
		case "self-hosted":
			return ["Single organization", "Docker deployment", "Full data sovereignty", "Community support"];
		default:
			return [];
	}
}
