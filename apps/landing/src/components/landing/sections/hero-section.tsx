      ...
      {task.labels.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-1.5">
          {visibleLabels.map((l) => (
            <span
              key={l.name}
              className="inline-flex items-center gap-1 text-[10px] py-0 px-1.5 rounded-2xl bg-accent border border-border h-4.5"
            >
              <span
          ...
          {overflowCount > 0 && (
            <span className="inline-flex items-center text-[10px] py-0 px-1.5 rounded-2xl bg-accent border border-border text-muted-foreground h-4.5">
              +{overflowCount}
            </span>
          )}
        </div>
      ...
      <div className="flex items-center justify-between mt-1">
        <div className="flex items-center gap-1.5">
          ...
          <span className="inline-flex items-center gap-1 text-[10px] text-muted-foreground">
            ...
            <span className="truncate max-w-15">{task.category.name}</span>
          </span>
          {/* Release */}
          {task.release && (
              ...
              <div className="grid grid-cols-4">
                {statuses.map((s, i) => {
                  ...
                  return (
                    <div
                      key={s}
                      className={`p-2 space-y-2 min-h-20 ${i < statuses.length - 1 ? "border-r border-border" : ""}`}
                    >
  ...
  return (
    ...
    >
          ...
          {visibleLabels.map((l) => (
            <span key={l.name} className="inline-flex items-center gap-1">
              <span
                ...
                style={{ backgroundColor: l.color }}
              />
              <span className="truncate max-w-35">{l.name}</span>
            </span>
          ))}
      ...
      <button
        ...
        className={`flex flex-col items-center justify-center w-13 h-14 rounded-lg border text-sm font-semibold transition-all shrink-0 ${
          voted
  ...
  return (
    <section className="relative min-h-[95dvh] flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0">
        <div className="absolute top-0 left-1/4 w-150 h-150 bg-primary/8 rounded-full blur-[120px]" />
        <div className="absolute bottom-0 right-1/4 w-125 h-125 bg-primary/5 rounded-full blur-[100px]" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-200 h-100 bg-primary/3 rounded-full blur-[150px]" />
      </div>
      ...
      >
        ...
        {/* Glow */}
        <div className="absolute -inset-4 bg-linear-to-r from-primary/8 via-primary/3 to-primary/8 rounded-3xl blur-2xl -z-10" />
      </motion.div>
    </section>
  );
}
