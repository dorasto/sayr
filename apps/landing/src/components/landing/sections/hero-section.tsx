...
const allTasks: TaskData[] = [
	{
		...
		description: "Add full dark mode support across all dashboard views with system preference detection.",
		status: "in-progress",
		priority: "high",
		visible: "public",
	...
	},
	{
		...
		description: "Support SAML 2.0 and OIDC for enterprise single sign-on integration with identity providers.",
		status: "in-progress",
		priority: "medium",
		visible: "public",
		category: { name: "Feature Request", color: "#7c3aed" },
	...
	},
	{
		...
		description: "Notify users via email when tasks they voted on change status.",
		status: "todo",
		priority: "high",
		visible: "public",
		category: { name: "Feature Request", color: "#7c3aed" },
	...
	},
	{
		...
		description: "Allow exporting task lists and analytics data to CSV format for external reporting.",
		status: "backlog",
		priority: "low",
		visible: "public",
		category: { name: "Feature Request", color: "#7c3aed" },
	...
	},
	{
		...
		description: "Implement configurable rate limits at the organization level to prevent abuse.",
		status: "backlog",
		priority: "medium",
		visible: "public",
		category: { name: "Feature Request", color: "#7c3aed" },
	...
	},
	{
		...
		description: "Two-way synchronization between Sayr tasks and GitHub issues.",
		status: "done",
		priority: "high",
		visible: "public",
		category: { name: "Feature Request", color: "#7c3aed" },
	...
	},
	{
		...
		description: "Send webhook payloads when tasks are created, updated, or deleted.",
		status: "todo",
		priority: "medium",
		visible: "public",
		category: { name: "Feature Request", color: "#7c3aed" },
	...
	},
	{
		...
		description: "Select multiple tasks and apply status changes, label assignments, or deletions in bulk.",
		status: "todo",
		priority: "low",
		visible: "public",
		category: { name: "Feature Request", color: "#7c3aed" },
...
function getTasksByPriorityAndStatus(priority: Priority, status: Status, tasks: TaskData[]) {
	...
	return (
		...
		>
			...
			<div className="flex items-center justify-between mb-1">
				<span className="text-[10px] text-muted-foreground font-mono">#{task.shortId}</span>
			...
			{/* Title */}
			<p className="font-medium text-xs leading-tight line-clamp-2 mb-1.5">{task.title}</p>
					...
					return (
						<div key={p}>
							...
							<div className="flex items-center gap-2 px-3 py-1.5 bg-accent border-b border-border">
								...
								<span className="text-[10px] text-muted-foreground">{rowTasks.length}</span>
							</div>
	...
	return (
		...
		>
			...
			<div className="flex-1 min-w-0">
				<h3 className="text-base font-bold leading-snug mb-0.5">{task.title}</h3>
				{task.description && (
					<p className="text-sm text-muted-foreground line-clamp-2 mb-1.5">{task.description}</p>
				)}
				{/* Metadata row */}
				<div className="flex items-center gap-2.5 flex-wrap text-xs text-muted-foreground">
					...
					{/* Category */}
					<span className="inline-flex items-center gap-1">
						<span className="size-2.5 rounded-full shrink-0" style={{ backgroundColor: task.category.color }} />
						{task.category.name}
					</span>
					{/* Short ID */}
			...
			>
				...
				<span className="leading-none">{voted ? task.votes + 1 : task.votes}</span>
			</button>
		</motion.div>
	);
}
			...
			<div className="relative z-10 w-full max-w-(--breakpoint-lg) mx-auto px-6 pt-16 pb-8 text-center">
				<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
					<Badge
						...
						render={
							<a href="https://platform.sayr.io/releases/0.3.0" target="_blank" rel="noreferrer">
				...
				>
					Sayr is an source-available project management platform with a public-facing portal. Your team works
					internally while users submit feedback, vote on features, and track progress — all from the same tool.
				</motion.p>
