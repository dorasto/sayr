...
function FeatureValue({ value, featureName }: { value: TierValue; featureName: string }) {
	if (value === false) {
		return (
		...
		<div className="w-full">
			...
			<section className="py-16 sm:py-24 px-6">
				<div className="mx-auto max-w-4xl text-center">
					...
					<p className="mt-4 text-lg text-muted-foreground max-w-2xl mx-auto">
						Start free with up to 5 members. Upgrade when your team grows. Self-host for full control.
					</p>
				</div>
			</section>
									...
									<div className="flex items-start justify-between gap-4 mb-3">
										<div>
											...
											{tier.price === null || tier.price === 0 ? (
												<p className="text-2xl font-semibold invisible select-none" aria-hidden="true">
													—
												</p>
					...
					<div className="md:hidden">
						{pricingData.featureGroups.map((group) => (
							<div key={group.name} className="mb-6">
								<h3 className="text-sm font-semibold text-foreground px-1 mb-2">{group.name}</h3>
								<div className="rounded-xl border bg-card divide-y divide-border overflow-hidden">
									{group.features.map((feature) => {
										const value = feature.tiers[activeTierId as keyof typeof feature.tiers];
										return (
											<div key={feature.name} className="px-4 py-3">
												<FeatureValue value={value} featureName={feature.name} />
											</div>
										);
									})}
								</div>
					...
					<div className="hidden md:block">
						...
						<div className="grid grid-cols-4 gap-0 sticky top-15 border-b">
							{tiers.map((tier) => (
								<div
									key={tier.id}
									className={cn("px-3 bg-background", tier.highlighted && "bg-card rounded-t-lg")}
								>
									<div className="flex flex-col gap-1 p-3 pb-4">
										<h2 className="text-xl! font-semibold">{tier.name}</h2>
										{tier.price === null || tier.price === 0 ? (
											<p className="text-lg font-semibold invisible select-none" aria-hidden="true">
												—
											</p>
										) : typeof tier.price === "number" ? (
											<div className="flex items-baseline gap-1">
												...
												{tier.priceSuffix && (
													<span className="text-xs text-muted-foreground">{tier.priceSuffix}</span>
												)}
											</div>
										) : (
							...
							<div key={group.name}>
								...
								<div className="grid grid-cols-4 gap-0">
									{tiers.map((tier, i) => (
										<div
											key={tier.id}
											className={cn("px-6 py-3 bg-background", tier.highlighted && "bg-card")}
										>
											{i === 0 && <h3 className="text-sm font-semibold text-foreground">{group.name}</h3>}
										</div>
									))}
								</div>
						...
						<div className="grid grid-cols-4 gap-0">
							{tiers.map((tier) => (
								<div key={tier.id} className={cn(tier.highlighted && "rounded-b-lg h-4 bg-card")} />
							))}
						</div>
					</div>

					<p className="text-center mt-10 text-sm text-muted-foreground">
						All prices are per organization. Only internal members count as seats &mdash; public users who vote
						and comment are always free.
					</p>
				</div>
			</section>
			...
			<section className="px-6 pb-24">
				<div className="mx-auto max-w-2xl">
					<h2 className="text-2xl font-semibold tracking-tight mb-8 text-center">Common questions</h2>
					<div className="space-y-2">
						{pricingFaqs.map((faq, i) => (
							<div key={faq.q} className="rounded-xl border bg-card overflow-hidden">
								<button
									type="button"
			...
			<section className="px-6 pb-24">
				<div className="mx-auto max-w-2xl text-center">
					...
					<div className="flex items-center justify-center gap-3">
						...
						/>
						<Button variant="outline" className="rounded-full" render={<a href="/docs">Read the Docs</a>} />
					</div>
				</div>
			</section>
		</div>
