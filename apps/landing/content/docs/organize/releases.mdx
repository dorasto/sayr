...
Members with the **Manage releases** permission can also manage releases from **Settings → Releases**:
...
Creating, editing, publishing, and deleting releases needs the **Manage releases** [team permission](/docs/organizations/members-and-teams) — organization creators and members of an Administrator team always have it. All members can view releases and assign tasks to them.
</Callout>
## Release Properties
...
<Callout type="warn" title="Auto-Close Warning">
...
Deleting a release needs the **Manage releases** permission.
...
| Create releases | Members with **Manage releases** |
| Edit releases | Members with **Manage releases** |
| Delete releases | Members with **Manage releases** |
| Mark as released | Members with **Manage releases** |
| Assign tasks to releases | All organization members |

## Managing Releases from the CLI and API

Everything above can also be done from your terminal or a script with the [Sayr CLI](/docs/cli/releases): create, edit, publish, and delete releases, manage their labels, post status updates and comments, and link GitHub pull requests. Personal access tokens reach the same actions through the `/v1/me/releases` routes of the [public API](/docs/api/overview).

```bash
sayr releases create "v1.2.0" --target-date 2026-10-31
sayr task update 123 --release v1-2-0
sayr releases publish v1-2-0 --yes
```

Changing a release with a personal access token needs the **Manage releases** scope (`content.manageReleases`) on the key, and the key's owner needs the **Manage releases** permission too — a key can never do more than its owner. Publishing closes the release's open tasks, just like marking it as released in the app, and neither publishing nor deleting can be undone, so the CLI asks for confirmation (or `--yes` when there's no terminal).
