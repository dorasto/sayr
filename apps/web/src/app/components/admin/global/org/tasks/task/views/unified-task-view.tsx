...
import { updateAssigneesToTaskAction, updateTaskAction } from "@/app/lib/fetches/task";
...
export function UnifiedTaskView({
		...
		if (changedItem) {
			...
			if (grouping === "status") {
				...
				await runWithToast(
					...
					() => updateTaskAction(organization.id, taskId, { category: categoryId }, wsClientId)
				);
			}
		}
	};
	...
	return (
		<div className="h-full overflow-x-auto rounded">
			{viewMode === "kanban" ? (
				<KanbanProvider columns={columns} data={kanbanData} onDataChange={handleKanbanDataChange}>
					{(column) => (
						<KanbanBoard
							key={column.id}
							id={column.id}
						...
						>
							<KanbanHeader className="pb-2 flex items-center justify-between bg-muted border-0 rounded-lg shrink-0 min-h-[42px]">
								<div className="flex items-center gap-2">
									{column.icon && (
										<span className={cn("text-sm font-medium", column.accentClassName)}>{column.icon}</span>
									)}
