 ...
 *
 * Client-side upload events (success/failure) are logged with Axiom.
 *
...
export async function uploadFile(
	...
	try {
		...
		logger.info("client.upload.start", {
		...
		const res = await fetch(`${process.env.NEXT_PUBLIC_EXTERNAL_API_URL}/file`, {
			method: "PUT",
			body: formData,
			credentials: "include",
			headers: {
		...
		if (!res.ok) {
			...
			logger.error("client.upload.failed", { fileName: file.name, error: msg });
			throw new Error(msg);
		}
		...
		logger.info("client.upload.success", {
		...
		logger.error("client.upload.error", {
 ...
 *
 * Client-side delete events (start/success/error) are logged with Axiom.
 *
...
export async function deleteFile(fileUrl: string): Promise<{
	...
	try {
		...
		if (!res.ok) {
			...
			logger.error("client.delete.failed", { fileUrl, error: msg });
			throw new Error(msg);
		}
